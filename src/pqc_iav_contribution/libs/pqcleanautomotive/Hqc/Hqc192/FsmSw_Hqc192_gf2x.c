...
*
*/
static void hqc192_base_mul(uint64 *const c, uint64 a, uint64 b)
{
  uint64 h = 0;
  uint64 l = 0;
  uint64 g;
...
static void hqc192_karatsuba(uint64 *const o, const uint64 *const a, const uint64 *const b, uint32 size, uint64 *stack)
{
  ...
  else
  {
    ...
    hqc192_karatsuba(o, a, b, size_l, stack_tmp);
    /* polyspace +2 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    hqc192_karatsuba(tmp2, ah, bh, size_h, stack_tmp);

    hqc192_karatsuba_add1(alh, blh, a, b, size_l, size_h);
    /* polyspace +2 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    hqc192_karatsuba(tmp1, alh, blh, size_l, stack_tmp);

    hqc192_karatsuba_add2(o, tmp1, tmp2, size_l, size_h);
  }
} // end: hqc192_karatsuba
...
*
*/
static void hqc192_gf2x_gf_reduce(uint64 *const o, const uint64 *const a)
{
  uint64 r;
  uint64 carry;
  ...
  o[HQC192_VEC_N_SIZE_64 - 1] &= (uint64)HQC192_RED_MASK;
} // end: hqc192_gf2x_gf_reduce
...
{
  uint64 stack[(uint32)HQC192_VEC_N_SIZE_64 << 3]   = {0};
  uint64 o_karat[(uint32)HQC192_VEC_N_SIZE_64 << 1] = {0};

  hqc192_karatsuba(o_karat, v1, v2, HQC192_VEC_N_SIZE_64, stack);
  hqc192_gf2x_gf_reduce(o, o_karat);
} // end: FsmSw_Hqc192_Vect_Mul
