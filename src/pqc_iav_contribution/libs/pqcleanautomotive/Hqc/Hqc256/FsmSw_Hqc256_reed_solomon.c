 ...
 ** @{ */
/** \addtogroup FsmSw_Hqc256_reed_solomon
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_Hqc256_reed_solomon.c
...
/*====================================================================================================================*/
/**
* \brief Computes 2 * HQC256_PARAM_DELTA syndromes
*
* \param[out]   syndromes Array of size 2 * HQC256_PARAM_DELTA receiving the computed syndromes
* \param[in]    cdw Array of size PARAM_N1 storing the received vector
*
*/
static void compute_syndromes_256(uint16 *const syndromes, const uint8 *const cdw)
{
  for (uint8 i = 0; i < (2 * HQC256_PARAM_DELTA); ++i)
  {
    for (uint8 j = 1; j < HQC256_PARAM_N1; ++j)
    {
      syndromes[i] ^= FsmSw_Hqc256_Gf_Mul(cdw[j], alpha_ij_pow_256[i][j - 1]);
    }
    syndromes[i] ^= cdw[0];
  }
...
* We can properly correct only if the degree of sigma does not exceed HQC256_PARAM_DELTA.
* This means only the first HQC256_PARAM_DELTA + 1 coefficients of sigma are of value
* and we only need to save its first HQC256_PARAM_DELTA - 1 coefficients.
*
* \returns      the degree of the ELP sigma
* \param[out]   sigma Array of size (at least) HQC256_PARAM_DELTA receiving the ELP
* \param[in]    syndromes Array of size (at least) 2*HQC256_PARAM_DELTA storing the syndromes
*
*/
static uint16 hqc256_compute_elp(uint16 *const sigma, const uint16 *const syndromes)
{
  uint16 deg_sigma                              = 0;
  uint16 deg_sigma_p                            = 0;
  uint16 deg_sigma_copy                         = 0;
  uint16 sigma_copy[HQC256_PARAM_DELTA + 1]     = {0};
  uint16 X_sigma_p[HQC256_PARAM_DELTA + 1]      = {0};
  X_sigma_p[HQC256_X_SIGMA_P_2ND_ELEMENT_INDEX] = 1;
  ...
  for (mu = 0; (mu < (2 * HQC256_PARAM_DELTA)); ++mu)
  {
    ...
    FsmSw_CommonLib_MemCpy(sigma_copy, sigma, 2 * (HQC256_PARAM_DELTA));
    deg_sigma_copy = deg_sigma;

    dd = FsmSw_Hqc256_Gf_Mul(d, FsmSw_Hqc256_Gf_Inverse(d_p));
    ...
    pp ^= mask12 & (mu ^ pp);
    d_p ^= mask12 & (d ^ d_p);
    for (i = HQC256_PARAM_DELTA; i > 0; --i)
    {
    ...
    for (i = 1; (i <= (mu + 1)) && (i <= HQC256_PARAM_DELTA); ++i)
    {
      d ^= FsmSw_Hqc256_Gf_Mul(sigma[i], syndromes[(mu + 1) - i]);
    }
  }
...
*
* See function FsmSw_Hqc256_fft for more details.
*
* \param[out]   sigma Array of 2^HQC256_PARAM_M elements receiving the error polynomial
* \param[out]   error_compact Array of HQC256_PARAM_DELTA + PARAM_N1 elements receiving a compact representation of the vector error
* \param[in]    sigma Array of 2^HQC256_PARAM_FFT elements storing the error locator polynomial
*
*/
static void compute_roots_256(uint8 *const err, const uint16 *const sigma)
{
  uint16 w[(uint32)1U << HQC256_PARAM_M] = {0};

  FsmSw_Hqc256_Fft(w, sigma, HQC256_PARAM_DELTA + 1);
  FsmSw_Hqc256_Fft_Retrieve_Error_Poly(err, w);
} // end: compute_roots
...
*
* \param[out]   z Array of HQC256_PARAM_DELTA + 1 elements receiving the polynomial z(x)
* \param[in]    sigma Array of 2^HQC256_PARAM_FFT elements storing the error locator polynomial
* \param[in]    degree Integer that is the degree of polynomial sigma
* \param[in]    syndromes Array of 2 * HQC256_PARAM_DELTA storing the syndromes
*
*/
static void hqc256_compute_z_poly(uint16 *const z, const uint16 *const sigma, uint16 degree,
                                  const uint16 *const syndromes)
{
  ...
  {
    mask = (((uint16)((i - degree - 1) & 0xFFFFU) >> 15) == 1) ? 0xFFFFU : 0x0000U;
    z[i] = mask & sigma[i];
  }
  ...
  for (i = 2; i <= HQC256_PARAM_DELTA; ++i)
  {
    ...
    for (j = 1; j < i; ++j)
    {
      z[i] ^= mask & FsmSw_Hqc256_Gf_Mul(sigma[j], syndromes[i - j - 1]);
    }
  }
} // end: compute_z_poly
...
* See \cite lin1983error (Chapter 6 - BCH Codes) for more details.
*
* \param[out]   error_values Array of HQC256_PARAM_DELTA elements receiving the error values
* \param[in]    z Array of HQC256_PARAM_DELTA + 1 elements storing the polynomial z(x)
* \param[in]    z_degree Integer that is the degree of polynomial z(x)
* \param[in]    error_compact Array of HQC256_PARAM_DELTA + PARAM_N1 storing compact representation of the error
*
*/
static void compute_error_values_256(uint16 *const error_values, const uint16 *const z, const uint8 *const err)
{
  uint16 beta_j[HQC256_PARAM_DELTA] = {0};
  uint16 e_j[HQC256_PARAM_DELTA]    = {0};
  ...
  delta_counter = 0;
  for (uint8 i = 0; i < HQC256_PARAM_N1; i++)
  {
    found = 0;
    mask1 = (err[i] > 0) ? 0xFFFFU : 0x0000U; // err[i] != 0
    for (uint8 j = 0; j < HQC256_PARAM_DELTA; j++)
    {
      mask2 = ~((uint16)((~FsmSw_Convert_u16_to_u32(j ^ delta_counter) + 1U) >> 15));
  ...
  for (uint8 i = 0; i < HQC256_PARAM_DELTA; ++i)
  {
    tmp1            = 1;
    tmp2            = 1;
    inverse         = FsmSw_Hqc256_Gf_Inverse(beta_j[i]);
    inverse_power_j = 1;

    for (uint8 j = 1; j <= HQC256_PARAM_DELTA; ++j)
    {
      inverse_power_j = FsmSw_Hqc256_Gf_Mul(inverse_power_j, inverse);
      tmp1 ^= FsmSw_Hqc256_Gf_Mul(inverse_power_j, z[j]);
    }
    for (uint8 k = 1; k < HQC256_PARAM_DELTA; ++k)
    {
      tmp2 = FsmSw_Hqc256_Gf_Mul(tmp2, (1U ^ FsmSw_Hqc256_Gf_Mul(inverse, beta_j[(i + k) % HQC256_PARAM_DELTA])));
    }
    mask1  = (i < delta_real_value) ? 0xFFFFU : 0x0000U;
    e_j[i] = mask1 & FsmSw_Hqc256_Gf_Mul(tmp1, FsmSw_Hqc256_Gf_Inverse(tmp2));
  }
...
* \param[in]    error_values Array of HQC256_PARAM_DELTA elements storing the error values
*
*/
static void hqc256_correct_errors(uint8 *const cdw, const uint16 *const error_values)
{
  for (uint8 i = 0; i < HQC256_PARAM_N1; ++i)
  {
...
/*====================================================================================================================*/
/**
* \brief Encodes a message message of HQC256_PARAM_K bits to a Reed-Solomon codeword codeword of PARAM_N1 bytes
*
* Following @cite lin1983error (Chapter 4 - Cyclic Codes),
* We perform a systematic encoding using a linear (PARAM_N1 - HQC256_PARAM_K)-stage shift register
* with feedback connections based on the generator polynomial PARAM_RS_POLY of the Reed-Solomon code.
*
* \param[out]   cdw Array of size HQC256_VEC_N1_SIZE_64 receiving the encoded message
* \param[in]    msg Array of size HQC256_VEC_K_SIZE_64 storing the message
*
*/
void FsmSw_Hqc256_Reed_Solomon_Encode(uint8 *const cdw, const uint8 *const msg)
{
  ...
  for (uint8 i = 0; i < HQC256_PARAM_K; ++i)
  {
    gate_value = msg[HQC256_PARAM_K - 1 - i] ^ cdw[HQC256_PARAM_N1 - HQC256_PARAM_K - 1];

    for (uint8 j = 0; j < HQC256_PARAM_G; ++j)
    {
  ...
  FsmSw_CommonLib_MemCpy(&cdw[HQC256_PARAM_N1 - HQC256_PARAM_K], msg, HQC256_PARAM_K);
} // end: FsmSw_Hqc256_Reed_Solomon_Encode
...
*    <ol>
*    <li> The first step, is the computation of the 2*HQC256_PARAM_DELTA syndromes.
...
*
*
* \param[out]   msg Array of size HQC256_VEC_K_SIZE_64 receiving the decoded message
* \param[in]    cdw Array of size HQC256_VEC_N1_SIZE_64 storing the received word
*
*/
void FsmSw_Hqc256_Reed_Solomon_Decode(uint8 *const msg, uint8 *const cdw)
{
  uint16 syndromes[2 * HQC256_PARAM_DELTA] = {0};
  uint16 sigma[1U << HQC256_PARAM_FFT]     = {0};
  uint8 err[(uint32)1U << HQC256_PARAM_M]  = {0};
  uint16 z[HQC256_PARAM_N1]                = {0};
  uint16 error_values[HQC256_PARAM_N1]     = {0};
  uint16 deg;

  // Calculate the 2*HQC256_PARAM_DELTA syndromes
  compute_syndromes_256(syndromes, cdw);

  // Compute the error locator polynomial sigma
  // Sigma's degree is at most HQC256_PARAM_DELTA but the FFT requires the extra room
  deg = hqc256_compute_elp(sigma, syndromes);

  // Compute the error polynomial error
  compute_roots_256(err, sigma);

  // Compute the polynomial z(x)
  hqc256_compute_z_poly(z, sigma, deg, syndromes);

  // Compute the error values
  compute_error_values_256(error_values, z, err);

  // Correct the errors
  hqc256_correct_errors(cdw, error_values);

  // Retrieve the message from the decoded codeword
  FsmSw_CommonLib_MemCpy(msg, &cdw[HQC256_PARAM_G - 1], HQC256_PARAM_K);
} // end: FsmSw_Hqc256_Reed_Solomon_Decode
