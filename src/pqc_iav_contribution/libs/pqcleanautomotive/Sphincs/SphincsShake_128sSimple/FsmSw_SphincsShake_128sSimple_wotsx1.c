 ...
 ** @{ */
/** \addtogroup SphincsShake_128sSimple_wotsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128sSimple_wotsx1.c
* \brief  description of FsmSw_SphincsShake_128sSimple_wotsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_128sSimple_Wots_GenLeafX1(uint8 *const dest, const sphincs_shake_128s_ctx *const ctx,
                                                  uint32 leaf_idx, void *const v_info)
{
  ...
  /* polyspace +2 MISRA2012:11.5 [Justified:]"Necessary conversion from void* to object* for functionality.
    Ensured proper alignment and validity." */
  Fsmsw_Sphincsshake_128sSimple_LeafInfoX1_T *info          = v_info;
  uint32 *const leaf_addr                                   = info->leaf_addr;
  uint32 *const pk_addr                                     = info->pk_addr;
  uint32 i                                                  = 0;
  uint32 k                                                  = 0;
  uint8 pk_buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_BYTES] = {0};
  ...
  FsmSw_SphincsShake_128sSimple_set_keypair_addr(leaf_addr, leaf_idx);
  FsmSw_SphincsShake_128sSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN; i++)
  {
    ...
    FsmSw_SphincsShake_128sSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < FSMSW_SPHINCS_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        FsmSw_CommonLib_MemCpy(&info->wots_sig[i * FSMSW_SPHINCSSHAKE_128SSIMPLE_N], buffer,
                               FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_W - 1u)
      {
        break;
      }
      ...
      FsmSw_SphincsShake_128sSimple_Thash(buffer, buffer, 1, ctx, leaf_addr);
    }

    buffer = &buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_N];
  }

  /* Do the final FsmSw_SphincsShake_128sSimple_Thash to generate the public keys */
  FsmSw_SphincsShake_128sSimple_Thash(dest, pk_buffer, FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN, ctx, pk_addr);
} // end: FsmSw_SphincsShake_128sSimple_Wots_GenLeafX1
