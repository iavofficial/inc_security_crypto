...
/**********************************************************************************************************************/
static void fsmsw_sphincsshake_256ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                  ...
                                                                  const uint8 *const seed)
{
  ...
  FsmSw_CommonLib_MemCpy(sk, seed, FSMSW_SPHINCSSHAKE_256SSIMPLE_CRYPTO_SEEDBYTES);

  FsmSw_CommonLib_MemCpy(pk, &sk[2u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], FSMSW_SPHINCSSHAKE_256SSIMPLE_N);

  FsmSw_CommonLib_MemCpy(ctx.pub_seed, pk, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
  FsmSw_CommonLib_MemCpy(ctx.sk_seed, sk, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
...
{
  sphincs_shake_256s_ctx ctx                                = {{0}};
  const uint8 *const sk_prf                                 = &sk[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
  const uint8 *const pk                                     = &sk[2u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
  uint8 optrand[FSMSW_SPHINCSSHAKE_256SSIMPLE_N]            = {0};
  uint8 mhash[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_256SSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  ...
  FsmSw_SphincsShake_256sSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_BYTES];

  for (i = 0; i < FSMSW_SPHINCSSHAKE_256SSIMPLE_D; i++)
  {
    ...
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT;
  }
...
{
  sphincs_shake_256s_ctx ctx                                = {{0}};
  const uint8 *const pub_root                               = &pk[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
  uint8 mhash[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_256SSIMPLE_N]               = {0};
  ...
  FsmSw_SphincsShake_256sSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_256SSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsShake_256sSimple_copy_keypair_addr(wots_pk_addr, wots_addr);
    ...
    FsmSw_SphincsShake_256sSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    FsmSw_SphincsShake_256sSimple_Thash(leaf, wots_pk, FSMSW_SPHINCSSHAKE_256SSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    FsmSw_SphincsShake_256sSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                              FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
                                               ...
                                               const uint8 *const sk)
{
  ...
  (void)FsmSw_SphincsShake_256sSimple_Crypto_Sign_Signature(sm, &siglen, m, mlen, sk);
                                                     ...
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  if (smlen < FSMSW_SPHINCSSHAKE_256SSIMPLE_BYTES)
  {
