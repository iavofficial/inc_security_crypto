 ...
 ** @{ */
/** \addtogroup SphincsSha2_256fSimple_wotsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256fSimple_wotsx1.c
* \brief  description of FsmSw_SphincsSha2_256fSimple_wotsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsSha2_256fSimple_Wots_GenLeafX1(uint8 *const dest, const sphincs_sha2_256f_ctx *const ctx,
                                                 uint32 leaf_idx, void *const v_info)
{
  ...
  uint32 i                                                 = 0;
  uint32 k                                                 = 0;
  uint8 pk_buffer[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES] = {0};
  ...
  FsmSw_SphincsSha2_256fSimple_set_keypair_addr(leaf_addr, leaf_idx);
  FsmSw_SphincsSha2_256fSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN; i++)
  {
    ...
    FsmSw_SphincsSha2_256fSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < FSMSW_SPHINCS_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        FsmSw_CommonLib_MemCpy(&info->wots_sig[i * FSMSW_SPHINCSSHA2_256FSIMPLE_N], buffer,
                               FSMSW_SPHINCSSHA2_256FSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_W - 1u)
      {
        break;
      }
      ...
      FsmSw_SphincsSha2_256fSimple_Thash(buffer, buffer, 1, ctx, leaf_addr);
    }

    buffer = &buffer[FSMSW_SPHINCSSHA2_256FSIMPLE_N];
  }

  /* Do the final thash to generate the public keys */
  FsmSw_SphincsSha2_256fSimple_Thash(dest, pk_buffer, FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN, ctx, pk_addr);
} // end: FsmSw_SphincsSha2_256fSimple_Wots_GenLeafX1
