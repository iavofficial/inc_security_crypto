 ...
 ** @{ */
/** \addtogroup SphincsShake_128fSimple_wotsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128fSimple_wotsx1.c
* \brief  description of FsmSw_SphincsShake_128fSimple_wotsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_128fSimple_Wots_GenLeafX1(uint8 *const dest, const sphincs_shake_128f_ctx *const ctx,
                                                  uint32 leaf_idx, void *const v_info)
{
  ...
  /* polyspace +2 MISRA2012:11.5 [Justified:]"Necessary conversion from void* to object* for functionality.
    Ensured proper alignment and validity." */
  Fsmsw_Sphincsshake_128fSimple_LeafInfoX1_T *info          = v_info;
  uint32 *const leaf_addr                                   = info->leaf_addr;
  uint32 *const pk_addr                                     = info->pk_addr;
  uint32 i                                                  = 0;
  uint32 k                                                  = 0;
  uint8 pk_buffer[FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_BYTES] = {0};
  ...
  FsmSw_SphincsShake_128fSimple_set_keypair_addr(leaf_addr, leaf_idx);
  FsmSw_SphincsShake_128fSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_LEN; i++)
  {
    ...
    FsmSw_SphincsShake_128fSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < FSMSW_SPHINCS_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        FsmSw_CommonLib_MemCpy(&info->wots_sig[i * FSMSW_SPHINCSSHAKE_128FSIMPLE_N], buffer,
                               FSMSW_SPHINCSSHAKE_128FSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_W - 1u)
      {
        break;
      }
      ...
      FsmSw_SphincsShake_128fSimple_Thash(buffer, buffer, 1, ctx, leaf_addr);
    }

    buffer = &buffer[FSMSW_SPHINCSSHAKE_128FSIMPLE_N];
  }

  /* Do the final FsmSw_SphincsShake_128fSimple_Thash to generate the public keys */
  FsmSw_SphincsShake_128fSimple_Thash(dest, pk_buffer, FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_LEN, ctx, pk_addr);
} // end: FsmSw_SphincsShake_128fSimple_Wots_GenLeafX1
