...
/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_192sSimple_sign.c
* \brief  description of FsmSw_SphincsShake_192sSimple_sign.c
*
* \details
*
*
...
/**********************************************************************************************************************/
static void fsmsw_sphincsshake_192ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                  ...
                                                                  const uint8 *const seed)
{
   ...
   * on the public seed. */
  FsmSw_SphincsShake_192sSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  FsmSw_SphincsShake_192sSimple_Merkle_GenRoot(&sk[3u * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], &ctx);

  /* cleanup */
  FsmSw_SphincsShake_192sSimple_4FreeHashFunction(&ctx);

  FsmSw_CommonLib_MemCpy(&pk[FSMSW_SPHINCSSHAKE_192SSIMPLE_N], &sk[3u * FSMSW_SPHINCSSHAKE_192SSIMPLE_N],
                         FSMSW_SPHINCSSHAKE_192SSIMPLE_N);

  return;
} // end: fsmsw_sphincsshake_192ssimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
 ...
 *
 */
void FsmSw_SphincsShake_192sSimple_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  uint8 seed[FSMSW_SPHINCSSHAKE_192SSIMPLE_CRYPTO_SEEDBYTES] = {0};
  (void)FsmSw_CommonLib_RandomBytes(seed, FSMSW_SPHINCSSHAKE_192SSIMPLE_CRYPTO_SEEDBYTES);
  fsmsw_sphincsshake_192ssimple_crypto_sign_SeedKeyPair(pk, sk, seed);

  return;
} // end: FsmSw_SphincsShake_192sSimple_Crypto_Sign_KeyPair
...
designed for use by other systems that aim to integrate the Sphincs." */
void FsmSw_SphincsShake_192sSimple_Crypto_Sign_Signature(uint8 *const sig, uint32 *const siglen, const uint8 *const m,
                                                         uint32 mlen, const uint8 *const sk)
{
  sphincs_shake_192s_ctx ctx = {{0}};

  const uint8 *const sk_prf = &sk[FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
  const uint8 *const pk     = &sk[2u * FSMSW_SPHINCSSHAKE_192SSIMPLE_N];

  uint8 optrand[FSMSW_SPHINCSSHAKE_192SSIMPLE_N]            = {0};
  uint8 mhash[FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_192SSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  ...
  FsmSw_SphincsShake_192sSimple_GenMessageRandom(sig_temp, sk_prf, optrand, m, mlen, &ctx);

  /* Derive the message digest and leaf index from R, PK and M. */
  FsmSw_SphincsShake_192sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_D; i++)
  {
    ...
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES +
                         (FSMSW_SPHINCSSHAKE_192SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_192SSIMPLE_N)];
...
/* polyspace +2 MISRA2012:8.7 [Justified:]"This is an interface function
designed for use by other systems that aim to integrate the Sphincs." */
uint8 FsmSw_SphincsShake_192sSimple_Crypto_Sign_Verify(const uint8 *const sig, uint32 siglen, const uint8 *const m,
                                                       uint32 mlen, const uint8 *const pk)
{
  sphincs_shake_192s_ctx ctx                                = {{0}};
  const uint8 *const pub_root                               = &pk[FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
  uint8 mhash[FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_192SSIMPLE_N]               = {0};
  ...
  FsmSw_SphincsShake_192sSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192SSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsShake_192sSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES];
    ...
    FsmSw_SphincsShake_192sSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
  ...
  return retVal;
} // end: FsmSw_SphincsShake_192sSimple_Crypto_Sign_Verify
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_192sSimple_Crypto_Sign(uint8 *const sm, uint32 *const smlen, const uint8 *const m, uint32 mlen,
                                               const uint8 *const sk)
{
  ...
  *smlen = siglen + mlen;
  ...
  return;
} // end: FsmSw_SphincsShake_192sSimple_Crypto_Sign
...
/* polyspace +2 MISRA2012:5.1 [Justified:]"The identifiers are distinct. The naming convention ensures clarity
and avoids confusion with other functions. Therefore, this warning is a false positive." */
uint8 FsmSw_SphincsShake_192sSimple_Crypto_Sign_Open(uint8 *const m, uint32 *const mlen, const uint8 *const sm,
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  *mlen = smlen - FSMSW_SPHINCSSHAKE_192SSIMPLE_BYTES;
  ...
  return retVal;
} // end: FsmSw_SphincsShake_192sSimple_Crypto_Sign_Open
