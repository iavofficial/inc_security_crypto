 ...
 ** @{ */
/** \addtogroup SphincsShake_256sSimple_utils
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_256sSimple_utils.c
* \brief  description of FsmSw_SphincsShake_256sSimple_utils.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_256sSimple_ComputeRoot(uint8 *const root, const uint8 *const leaf, uint32 leaf_idx,
                                               uint32 idx_offset, const uint8 *const auth_path, uint32 tree_height,
                                               const sphincs_shake_256s_ctx *const ctx, uint32 addr[8])
{
  uint32 i                                           = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N] = {0};
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], leaf, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
  }
  auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
  ...
  for (i = 0; i < (tree_height - 1u); i++)
  {
    ...
    if (0u < (leaf_idx_temp & 1u))
    {
      FsmSw_SphincsShake_256sSimple_Thash(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], buffer, 2u, ctx, addr);
      FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    }
    else
    {
      FsmSw_SphincsShake_256sSimple_Thash(buffer, buffer, 2, ctx, addr);
      FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    }
    auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
  }
  ...
  FsmSw_SphincsShake_SetTreeIndex(addr, leaf_idx_temp + idx_offset_temp);
  FsmSw_SphincsShake_256sSimple_Thash(root, buffer, 2, ctx, addr);
} // end: FsmSw_SphincsShake_256sSimple_ComputeRoot
...
/* polyspace +2 MISRA2012:5.1 [Justified:]"The identifiers are distinct. The naming convention ensures clarity
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_256sSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                            const sphincs_shake_256s_ctx *const ctx, uint32 leaf_idx, uint32 idx_offset,
                                            uint32 tree_height,
                                            void (*const gen_leaf)(uint8 *leaf, const sphincs_shake_256s_ctx *ctx,
                                                                   uint32 addr_idx, const uint32 tree_addr[8]),
                                            uint32 tree_addr[8])
{
  uint8 stack[(FSMSW_SPHINCSSHAKE_256SSIMPLE_TREEHASH_BUF_LEN + 1u) * FSMSW_SPHINCSSHAKE_256SSIMPLE_N] = {0};
  uint32 heights[FSMSW_SPHINCSSHAKE_256SSIMPLE_TREEHASH_BUF_LEN + 1u]                                  = {0};
  uint32 offset                                                                                        = 0;
  uint32 idx                                                                                           = 0;
  uint32 tree_idx                                                                                      = 0;
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsShake_256sSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                                          &stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                               &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                               FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
      }
    }
  }
  FsmSw_CommonLib_MemCpy(root, stack, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
} // end: FsmSw_SphincsShake_256sSimple_TreeHash
