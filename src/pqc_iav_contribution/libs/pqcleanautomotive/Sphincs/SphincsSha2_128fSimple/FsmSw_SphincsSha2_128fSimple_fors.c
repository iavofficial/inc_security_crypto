...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_128fsimple_fors_GenSk(uint8 *const sk, const sphincs_sha2_128f_ctx *const ctx,
                                                    const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_128fsimple_fors_SkToLeaf(uint8 *const leaf, const uint8 *const sk,
                                                       const sphincs_sha2_128f_ctx *const ctx,
                                                       const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_128fsimple_fors_GenLeafx1(uint8 *const leaf, const sphincs_sha2_128f_ctx *const ctx,
                                                        uint32 addr_idx, void *const info);
static void fsmsw_sphincssha2_128fsimple_fors_MessageToIndices(uint32 *const indices, const uint8 *const m);
/**********************************************************************************************************************/
                                                        ...
                                                        uint32 addr_idx, void *const info)
{
  ...
  fsmsw_sphincssha2_128fsimple_fors_GenSk(leaf, ctx, fors_leaf_addr);
...
static void fsmsw_sphincssha2_128fsimple_fors_MessageToIndices(uint32 *const indices, const uint8 *const m)
{
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_TREES; i++)
  {
    indices[i] = 0;
    for (j = 0; j < FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT; j++)
    {
...
{
  uint32 indices[FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_TREES]                               = {0};
  uint8 roots[FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_TREES * FSMSW_SPHINCSSHA2_128FSIMPLE_N] = {0};
  uint32 fors_tree_addr[FSMSW_SPHINCS_FORS_ADDR_SIZE]                                   = {0};
  Fsmsw_Sphincssha2_128fSimple_ForsGenLeafInfo fors_info                                = {{0}};
  uint32 *const fors_leaf_addr                                                          = fors_info.leaf_addrx;
  ...
  FsmSw_SphincsSha2_128fSimple_copy_keypair_addr(fors_tree_addr, fors_addr);
  FsmSw_SphincsSha2_128fSimple_copy_keypair_addr(fors_leaf_addr, fors_addr);

  FsmSw_SphincsSha2_128fSimple_copy_keypair_addr(fors_pk_addr, fors_addr);
  FsmSw_SphincsSha2_SetType(fors_pk_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSPK);

  fsmsw_sphincssha2_128fsimple_fors_MessageToIndices(indices, m);

  for (i = 0; i < FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_TREES; i++)
  {
    idx_offset = i * (1u << FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT);
    ...
    fsmsw_sphincssha2_128fsimple_fors_GenSk(sig_temp, ctx, fors_tree_addr);
    ...
    FsmSw_SphincsSha2_128fSimple_TreeHashX1(&roots[i * FSMSW_SPHINCSSHA2_128FSIMPLE_N], sig_temp, ctx, indices[i],
                                            idx_offset, FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT,
                                            fsmsw_sphincssha2_128fsimple_fors_GenLeafx1, fors_tree_addr, &fors_info);

    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128FSIMPLE_N * FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT];
  }
                                                 ...
                                                 const sphincs_sha2_128f_ctx *const ctx, const uint32 fors_addr[8])
{
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_TREES; i++)
  {
    ...
    fsmsw_sphincssha2_128fsimple_fors_SkToLeaf(leaf, sig_temp, ctx, fors_tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128FSIMPLE_N];

    /* Derive the corresponding root node of this tree. */
    FsmSw_SphincsSha2_128fSimple_ComputeRoot(&roots[i * FSMSW_SPHINCSSHA2_128FSIMPLE_N], leaf, indices[i], idx_offset,
                                             sig_temp, FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT, ctx, fors_tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128FSIMPLE_N * FSMSW_SPHINCSSHA2_128FSIMPLE_FORS_HEIGHT];
  }
