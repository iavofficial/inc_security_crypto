 ...
 ** @{ */
/** \addtogroup SphincsShake_256sSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_256sSimple_utilsx1.c
* \brief  description of FsmSw_SphincsShake_256sSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_256sSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              const sphincs_shake_256s_ctx *const ctx, uint32 leaf_idx,
                                              ...
                                              uint32 tree_addr[8], void *const info)
{
  ...
  uint8 stack[FSMSW_SPHINCSSHAKE_256SSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHAKE_256SSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[FSMSW_SPHINCSSHAKE_256SSIMPLE_N]. We do this to minimize the number of
     * copies needed during a FsmSw_SphincsShake_256sSimple_Thash */
    uint8 current[2u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
    gen_leaf(&current[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                               &current[FSMSW_SPHINCSSHAKE_256SSIMPLE_N], FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
      }
      ...
      FsmSw_SphincsShake_SetTreeHeight(tree_addr, h + 1u);
      FsmSw_SphincsShake_SetTreeIndex(tree_addr, (internal_idx / 2u) + internal_idx_offset);

      uint8 *const left = &stack[h * FSMSW_SPHINCSSHAKE_256SSIMPLE_N];
      FsmSw_CommonLib_MemCpy(&current[0], left, FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
      FsmSw_SphincsShake_256sSimple_Thash(&current[FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                                          &current[0u * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      FsmSw_CommonLib_MemCpy(&stack[h * FSMSW_SPHINCSSHAKE_256SSIMPLE_N], &current[FSMSW_SPHINCSSHAKE_256SSIMPLE_N],
                             FSMSW_SPHINCSSHAKE_256SSIMPLE_N);
    }
  }
} // end: FsmSw_SphincsShake_256sSimple_TreeHashX1
