 ...
 ** @{ */
/** \addtogroup SphincsShake_192sSimple_wotsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_192sSimple_wotsx1.c
* \brief  description of FsmSw_SphincsShake_192sSimple_wotsx1.c
*
* \details
*
*
                                                   ...
                                                   uint32 leaf_idx, void *const v_info)
{
  ...
  uint32 i                                                  = 0;
  uint32 k                                                  = 0;
  uint8 pk_buffer[FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_BYTES] = {0};
  ...
  FsmSw_SphincsShake_192sSimple_set_keypair_addr(leaf_addr, leaf_idx);
  FsmSw_SphincsShake_192sSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_LEN; i++)
  {
    ...
    FsmSw_SphincsShake_192sSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < FSMSW_SPHINCS_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        FsmSw_CommonLib_MemCpy(&info->wots_sig[i * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], buffer,
                               FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == FSMSW_SPHINCSSHAKE_192SSIMPLE_WOTS_W - 1u)
      {
        break;
      }
