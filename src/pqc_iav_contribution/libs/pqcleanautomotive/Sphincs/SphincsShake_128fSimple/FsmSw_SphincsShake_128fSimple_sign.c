...
/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128fSimple_sign.c
* \brief  description of FsmSw_SphincsShake_128fSimple_sign.c
*
* \details
*
*
...
/**********************************************************************************************************************/
static void fsmsw_sphincsshake_128fsimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
...
static void fsmsw_sphincsshake_128fsimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                  const uint8 *const seed)
{
  sphincs_shake_128f_ctx ctx = {{0}};

  /* Initialize SK_SEED, SK_PRF and PUB_SEED from seed. */
  FsmSw_CommonLib_MemCpy(sk, seed, FSMSW_SPHINCSSHAKE_128FSIMPLE_CRYPTO_SEEDBYTES);

  FsmSw_CommonLib_MemCpy(pk, &sk[2u * FSMSW_SPHINCSSHAKE_128FSIMPLE_N], FSMSW_SPHINCSSHAKE_128FSIMPLE_N);

  FsmSw_CommonLib_MemCpy(ctx.pub_seed, pk, FSMSW_SPHINCSSHAKE_128FSIMPLE_N);
  FsmSw_CommonLib_MemCpy(ctx.sk_seed, sk, FSMSW_SPHINCSSHAKE_128FSIMPLE_N);

  /* This hook allows the hash function instantiation to do whatever
     preparation or computation it needs, based on the public seed. */
  FsmSw_SphincsShake_128fSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  FsmSw_SphincsShake_128fSimple_Merkle_GenRoot(&sk[3u * FSMSW_SPHINCSSHAKE_128FSIMPLE_N], &ctx);

  /* cleanup */
  FsmSw_SphincsShake_128fSimple_1FreeHashFunction(&ctx);

  FsmSw_CommonLib_MemCpy(&pk[FSMSW_SPHINCSSHAKE_128FSIMPLE_N], &sk[3u * FSMSW_SPHINCSSHAKE_128FSIMPLE_N],
                         FSMSW_SPHINCSSHAKE_128FSIMPLE_N);

  return;
} // end: fsmsw_sphincsshake_128fsimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
...
{
  uint8 seed[FSMSW_SPHINCSSHAKE_128FSIMPLE_CRYPTO_SEEDBYTES] = {0};
  (void)FsmSw_CommonLib_RandomBytes(seed, FSMSW_SPHINCSSHAKE_128FSIMPLE_CRYPTO_SEEDBYTES);
  fsmsw_sphincsshake_128fsimple_crypto_sign_SeedKeyPair(pk, sk, seed);

  return;
} // end: FsmSw_SphincsShake_128fSimple_Crypto_Sign_KeyPair
                                                         ...
                                                         uint32 mlen, const uint8 *const sk)
{
  ...
  const uint8 *const sk_prf = &sk[FSMSW_SPHINCSSHAKE_128FSIMPLE_N];
  const uint8 *const pk     = &sk[2u * FSMSW_SPHINCSSHAKE_128FSIMPLE_N];

  uint8 optrand[FSMSW_SPHINCSSHAKE_128FSIMPLE_N]            = {0};
  uint8 mhash[FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_128FSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  ...
  (void)FsmSw_CommonLib_RandomBytes(optrand, FSMSW_SPHINCSSHAKE_128FSIMPLE_N);
  /* Compute the digest randomization value. */
  FsmSw_SphincsShake_128fSimple_GenMessageRandom(sig_temp, sk_prf, optrand, m, mlen, &ctx);

  /* Derive the message digest and leaf index from R, PK and M. */
  FsmSw_SphincsShake_128fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_N];

  FsmSw_SphincsShake_SetTreeAddr(wots_addr, tree);
  FsmSw_SphincsShake_128fSimple_set_keypair_addr(wots_addr, idx_leaf);

  /* Sign the message hash using FORS. */
  FsmSw_SphincsShake_128fSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_BYTES];

  for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsShake_128fSimple_Merkle_Sign(sig_temp, root, &ctx, wots_addr, tree_addr, idx_leaf);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_BYTES +
                         (FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_128FSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & ((1u << FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT) - 1u));
    tree     = tree >> FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT;
  }

  FsmSw_SphincsShake_128fSimple_1FreeHashFunction(&ctx);

  *siglen = FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES;

  return;
} // end: FsmSw_SphincsShake_128fSimple_Crypto_Sign_Signature
...
{
  sphincs_shake_128f_ctx ctx                                = {{0}};
  const uint8 *const pub_root                               = &pk[FSMSW_SPHINCSSHAKE_128FSIMPLE_N];
  uint8 mhash[FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[FSMSW_SPHINCSSHAKE_128FSIMPLE_N]               = {0};
  ...
  if (siglen != FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES)
  {
    retVal = ERR_NOT_OK;
  }
   ...
   * The additional FSMSW_SPHINCSSHAKE_128FSIMPLE_N is a result of the hash domain separator. */
  FsmSw_SphincsShake_128fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_N];
  ...
  FsmSw_SphincsShake_128fSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsShake_128fSimple_copy_keypair_addr(wots_pk_addr, wots_addr);
    ...
    FsmSw_SphincsShake_128fSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    FsmSw_SphincsShake_128fSimple_Thash(leaf, wots_pk, FSMSW_SPHINCSSHAKE_128FSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    FsmSw_SphincsShake_128fSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                              FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_128FSIMPLE_N];
  ...
  if (FsmSw_CommonLib_MemCmp(root, pub_root, FSMSW_SPHINCSSHAKE_128FSIMPLE_N) != 0u)
  {
    retVal = ERR_NOT_OK;
  }

  return retVal;
} // end: FsmSw_SphincsShake_128fSimple_Crypto_Sign_Verify
                                               ...
                                               const uint8 *const sk)
{
  ...
  (void)FsmSw_SphincsShake_128fSimple_Crypto_Sign_Signature(sm, &siglen, m, mlen, sk);

  FsmSw_CommonLib_MemMove(&sm[FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES], m, mlen);
  *smlen = siglen + mlen;

  return;
} // end: FsmSw_SphincsShake_128fSimple_Crypto_Sign
                                                     ...
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  /* The API caller does not necessarily know what size a signature should be
     but SPHINCS+ signatures are always exactly FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES. */
  if (smlen < FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES)
  {
  ...
  *mlen = smlen - FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES;

  if (FsmSw_SphincsShake_128fSimple_Crypto_Sign_Verify(sm, FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES,
                                                       &sm[FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES], *mlen, pk) != 0)
  {
    FsmSw_CommonLib_MemSet(m, 0, smlen);
    *mlen  = 0;
    retVal = ERR_NOT_OK;
  ...
  FsmSw_CommonLib_MemMove(m, &sm[FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES], *mlen);

  return retVal;
} // end: FsmSw_SphincsShake_128fSimple_Crypto_Sign_Open
