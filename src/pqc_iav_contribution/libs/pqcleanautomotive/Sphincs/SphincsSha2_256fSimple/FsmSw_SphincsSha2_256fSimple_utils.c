 ...
 ** @{ */
/** \addtogroup SphincsSha2_256fSimple_utils
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256fSimple_utils.c
* \brief  description of FsmSw_SphincsSha2_256fSimple_utils.c
*
* \details
*
*
...
{
  uint32 i                                          = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N] = {0};
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHA2_256FSIMPLE_N], leaf, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
...
void FsmSw_SphincsSha2_256fSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                           ...
                                           uint32 tree_addr[8])
{
  uint8 stack[(FSMSW_SPHINCSSHA2_256FSIMPLE_TREEHASH_BUF_LEN + 1u) * FSMSW_SPHINCSSHA2_256FSIMPLE_N] = {0};
  uint32 heights[FSMSW_SPHINCSSHA2_256FSIMPLE_TREEHASH_BUF_LEN + 1u]                                 = {0};
  uint32 offset                                                                                      = 0;
  uint32 idx                                                                                         = 0;
  uint32 tree_idx                                                                                    = 0;
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * FSMSW_SPHINCSSHA2_256FSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsSha2_256fSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                                         &stack[(offset - 2u) * FSMSW_SPHINCSSHA2_256FSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                               &stack[(offset - 1u) * FSMSW_SPHINCSSHA2_256FSIMPLE_N], FSMSW_SPHINCSSHA2_256FSIMPLE_N);
      }
    }
  }
  FsmSw_CommonLib_MemCpy(root, stack, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
} // end: FsmSw_SphincsSha2_256fSimple_TreeHash
