...
void FsmSw_Dilithium3_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  /* Matrix-vector multiplication */
  s1hat = s1;
  FsmSw_Dilithium3_Polyvecl_Ntt(&s1hat);
  FsmSw_Dilithium3_Polyvec_MatrixPointwiseMontgomery(&t1, mat, &s1hat);
  FsmSw_Dilithium3_Polyveck_Reduce(&t1);
  FsmSw_Dilithium3_Polyveck_InvnttTomont(&t1);

  /* Add error vector s2 */
  FsmSw_Dilithium3_Polyveck_Add(&t1, &t1, &s2);

  /* Extract t1 and write public key */
  FsmSw_Dilithium3_Polyveck_CAddQ(&t1);
  FsmSw_Dilithium3_Polyveck_Power2Round(&t1, &t0, &t1);
  FsmSw_Dilithium3_PackPk(pk, rho, &t1);
