...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_128sSimple_wotsx1.c
* \brief  description of FsmSw_SphincsSha2_128sSimple_wotsx1.c
*
* \details
*
*
                                                 ...
                                                 uint32 leaf_idx, void *const v_info)
{
  ...
  FsmSw_SphincsSha2_128sSimple_set_keypair_addr(leaf_addr, leaf_idx);
  FsmSw_SphincsSha2_128sSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_LEN; i++)
  {
    ...
    FsmSw_SphincsSha2_128sSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < FSMSW_SPHINCS_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        FsmSw_CommonLib_MemCpy(&info->wots_sig[i * FSMSW_SPHINCSSHA2_128SSIMPLE_N], buffer,
                               FSMSW_SPHINCSSHA2_128SSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_W - 1u)
      {
        break;
      }
      ...
      FsmSw_SphincsSha2_128sSimple_Thash(buffer, buffer, 1, ctx, leaf_addr);
    }

    buffer = &buffer[FSMSW_SPHINCSSHA2_128SSIMPLE_N];
  }

  /* Do the final thash to generate the public keys */
  FsmSw_SphincsSha2_128sSimple_Thash(dest, pk_buffer, FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_LEN, ctx, pk_addr);
} // end: FsmSw_SphincsSha2_128sSimple_Wots_GenLeafX1
