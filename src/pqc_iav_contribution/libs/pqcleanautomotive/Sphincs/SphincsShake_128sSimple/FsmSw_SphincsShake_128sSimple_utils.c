...
/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128sSimple_utils.c
* \brief  description of FsmSw_SphincsShake_128sSimple_utils.c
*
* \details
*
*
...
{
  uint32 i                                           = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHAKE_128SSIMPLE_N] = {0};
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], leaf, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
  }
  auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHAKE_128SSIMPLE_N];
  ...
  for (i = 0; i < (tree_height - 1u); i++)
  {
    ...
    if (0u < (leaf_idx_temp & 1u))
    {
      FsmSw_SphincsShake_128sSimple_Thash(&buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], buffer, 2u, ctx, addr);
      FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
    }
    else
    {
      FsmSw_SphincsShake_128sSimple_Thash(buffer, buffer, 2u, ctx, addr);
      FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
    }
    auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHAKE_128SSIMPLE_N];
  }
  ...
  FsmSw_SphincsShake_SetTreeIndex(addr, leaf_idx_temp + idx_offset_temp);
  FsmSw_SphincsShake_128sSimple_Thash(root, buffer, 2u, ctx, addr);
} // end: FsmSw_SphincsShake_128sSimple_ComputeRoot
 ...
 *        type (i.e. FSMSW_SPHINCS_ADDR_TYPE_HASHTREE or FSMSW_SPHINCS_ADDR_TYPE_FORSTREE). 
 *        Applies the offset idx_offset to indices before building addresses, so that it is 
 *        possible to continue counting indices across trees.
 * 
 * \param[out] uint8                       *root : t.b.d.
...
void FsmSw_SphincsShake_128sSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                            ...
                                            uint32 tree_addr[8])
{
  uint8 stack[(FSMSW_SPHINCSSHAKE_128SSIMPLE_TREEHASH_BUF_LEN + 1u) * FSMSW_SPHINCSSHAKE_128SSIMPLE_N] = {0};
  uint32 heights[FSMSW_SPHINCSSHAKE_128SSIMPLE_TREEHASH_BUF_LEN + 1u]                                  = {0};
  uint32 offset                                                                                        = 0;
  uint32 idx                                                                                           = 0;
  uint32 tree_idx                                                                                      = 0;
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * FSMSW_SPHINCSSHAKE_128SSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsShake_128sSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                                          &stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_128SSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                               &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                               FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
      }
    }
  }
  FsmSw_CommonLib_MemCpy(root, stack, FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
} // end: FsmSw_SphincsShake_128sSimple_TreeHash
