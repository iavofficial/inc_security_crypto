 ...
 ** @{ */
/** \addtogroup SphincsSha2_192sSimple_sign
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_192sSimple_sign.c
* \brief  description of FsmSw_SphincsSha2_192sSimple_sign.c
*
* \details
*
*
...
#include "FsmSw_SphincsSha2_192sSimple_FctWrapper.h"
#include "FsmSw_SphincsSha2_192sSimple_context.h"
#include "FsmSw_SphincsSha2_192sSimple_fors.h"
#include "FsmSw_SphincsSha2_192sSimple_hash.h"
#include "FsmSw_SphincsSha2_192sSimple_merkle.h"
...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_192ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
...
static void fsmsw_sphincssha2_192ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                 const uint8 *const seed)
{
  sphincs_sha2_192s_ctx ctx = {{0}};

  /* Initialize SK_SEED, SK_PRF and PUB_SEED from seed. */
  FsmSw_CommonLib_MemCpy(sk, seed, FSMSW_SPHINCSSHA2_192SSIMPLE_CRYPTO_SEEDBYTES);

  FsmSw_CommonLib_MemCpy(pk, &sk[2u * FSMSW_SPHINCSSHA2_192SSIMPLE_N], FSMSW_SPHINCSSHA2_192SSIMPLE_N);

  FsmSw_CommonLib_MemCpy(ctx.pub_seed, pk, FSMSW_SPHINCSSHA2_192SSIMPLE_N);
  FsmSw_CommonLib_MemCpy(ctx.sk_seed, sk, FSMSW_SPHINCSSHA2_192SSIMPLE_N);

  /* This hook allows the hash function instantiation to do whatever preparation or computation it needs, based on
   * the public seed. */
  FsmSw_SphincsSha2_192sSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  FsmSw_SphincsSha2_192sSimple_Merkle_GenRoot(&sk[3u * FSMSW_SPHINCSSHA2_192SSIMPLE_N], &ctx);

  FsmSw_CommonLib_MemCpy(&pk[FSMSW_SPHINCSSHA2_192SSIMPLE_N], &sk[3u * FSMSW_SPHINCSSHA2_192SSIMPLE_N],
...
{
  uint8 seed[FSMSW_SPHINCSSHA2_192SSIMPLE_CRYPTO_SEEDBYTES] = {0};
  (void)FsmSw_CommonLib_RandomBytes(seed, FSMSW_SPHINCSSHA2_192SSIMPLE_CRYPTO_SEEDBYTES);
                                                        ...
                                                        uint32 mlen, const uint8 *const sk)
{
  ...
  const uint8 *const sk_prf = &sk[FSMSW_SPHINCSSHA2_192SSIMPLE_N];
  const uint8 *const pk     = &sk[2u * FSMSW_SPHINCSSHA2_192SSIMPLE_N];

  uint8 optrand[FSMSW_SPHINCSSHA2_192SSIMPLE_N]            = {0};
  uint8 mhash[FSMSW_SPHINCSSHA2_192SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[FSMSW_SPHINCSSHA2_192SSIMPLE_N]               = {0};
  uint32 i                                                 = 0;
  uint64 tree                                              = 0;
  uint32 idx_leaf                                          = 0;
  ...
  (void)FsmSw_CommonLib_RandomBytes(optrand, FSMSW_SPHINCSSHA2_192SSIMPLE_N);
  /* Compute the digest randomization value. */
  FsmSw_SphincsSha2_192sSimple_GenMessageRandom(sig_temp, sk_prf, optrand, m, mlen, &ctx);

  /* Derive the message digest and leaf index from R, PK and M. */
  FsmSw_SphincsSha2_192sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_N];

  FsmSw_SphincsSha2_SetTreeAddr(wots_addr, tree);
  FsmSw_SphincsSha2_192sSimple_set_keypair_addr(wots_addr, idx_leaf);

  /* Sign the message hash using FORS. */
  FsmSw_SphincsSha2_192sSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_FORS_BYTES];

  for (i = 0; i < FSMSW_SPHINCSSHA2_192SSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsSha2_192sSimple_Merkle_Sign(sig_temp, root, &ctx, wots_addr, tree_addr, idx_leaf);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_BYTES +
                         (FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_192SSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT;
  }
...
{
  sphincs_sha2_192s_ctx ctx                                = {{0}};
  const uint8 *const pub_root                              = &pk[FSMSW_SPHINCSSHA2_192SSIMPLE_N];
  uint8 mhash[FSMSW_SPHINCSSHA2_192SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[FSMSW_SPHINCSSHA2_192SSIMPLE_N]               = {0};
  ...
  if (siglen != FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES)
  {
    retVal = ERR_NOT_OK;
  }
  ...
  /* Derive the message digest and leaf index from R || PK || M. The additional FSMSW_SPHINCSSHA2_192SSIMPLE_N is a
   * result of the hash domain separator. */
  FsmSw_SphincsSha2_192sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_N];
  ...
  FsmSw_SphincsSha2_192sSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_192SSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsSha2_192sSimple_copy_keypair_addr(wots_pk_addr, wots_addr);
    ...
    FsmSw_SphincsSha2_192sSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    FsmSw_SphincsSha2_192sSimple_Thash(leaf, wots_pk, FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    FsmSw_SphincsSha2_192sSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                             FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_192SSIMPLE_N];
  ...
  if (FsmSw_CommonLib_MemCmp(root, pub_root, FSMSW_SPHINCSSHA2_192SSIMPLE_N) != 0u)
  {
    retVal = ERR_NOT_OK;
  }
                                              ...
                                              const uint8 *const sk)
{
  ...
  (void)FsmSw_SphincsSha2_192sSimple_Crypto_Sign_Signature(sm, &siglen, m, mlen, sk);

  FsmSw_CommonLib_MemMove(&sm[FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES], m, mlen);
  *smlen = siglen + mlen;

  return;
} // end: FsmSw_SphincsSha2_192sSimple_Crypto_Sign
                                                    ...
                                                    uint32 smlen, const uint8 *const pk)
{
  ...
  /* The API caller does not necessarily know what size a signature should be
     but SPHINCS+ signatures are always exactly FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES. */
  if (smlen < FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES)
  {
  ...
  *mlen = smlen - FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES;
  ...
  FsmSw_CommonLib_MemMove(m, &sm[FSMSW_SPHINCSSHA2_192SSIMPLE_BYTES], *mlen);

  return retVal;
} // end: FsmSw_SphincsSha2_192sSimple_Crypto_Sign_Open
