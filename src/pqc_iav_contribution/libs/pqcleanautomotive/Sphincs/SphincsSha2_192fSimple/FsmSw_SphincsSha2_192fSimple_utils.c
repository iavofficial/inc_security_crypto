 ...
 ** @{ */
/** \addtogroup SphincsSha2_192fSimple_utils
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_192fSimple_utils.c
* \brief  description of FsmSw_SphincsSha2_192fSimple_utils.c
*
* \details
*
*
 ...
 *
 **********************************************************************************************************************/
void FsmSw_SphincsSha2_192fSimple_ComputeRoot(uint8 *const root, const uint8 *const leaf, uint32 leaf_idx,
                                              uint32 idx_offset, const uint8 *const auth_path, uint32 tree_height,
                                              const sphincs_sha2_192f_ctx *const ctx, uint32 addr[8])
{
  uint32 i                                          = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHA2_192FSIMPLE_N] = {0};
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHA2_192FSIMPLE_N], leaf, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHA2_192FSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
  }
  auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHA2_192FSIMPLE_N];
  ...
  for (i = 0; i < (tree_height - 1u); i++)
  {
    ...
    if (0u < (leaf_idx_temp & 1u))
    {
      FsmSw_SphincsSha2_192fSimple_Thash(&buffer[FSMSW_SPHINCSSHA2_192FSIMPLE_N], buffer, 2, ctx, addr);
      FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
    }
    else
    {
      FsmSw_SphincsSha2_192fSimple_Thash(buffer, buffer, 2, ctx, addr);
      FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHA2_192FSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
    }
    auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHA2_192FSIMPLE_N];
  }
  ...
  FsmSw_SphincsSha2_SetTreeIndex(addr, leaf_idx_temp + idx_offset_temp);
  FsmSw_SphincsSha2_192fSimple_Thash(root, buffer, 2, ctx, addr);
}
 ...
 *        idx_offset to indices before building addresses, so that it is possible to continue counting
 *        indices across trees.
 *
 ...
 */
void FsmSw_SphincsSha2_192fSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                           const sphincs_sha2_192f_ctx *const ctx, uint32 leaf_idx, uint32 idx_offset,
                                           uint32 tree_height,
                                           void (*const gen_leaf)(uint8 *leaf, const sphincs_sha2_192f_ctx *ctx,
                                                                  uint32 addr_idx, const uint32 tree_addr[8]),
                                           uint32 tree_addr[8])
{
  uint8 stack[(FSMSW_SPHINCSSHA2_192FSIMPLE_TREEHASH_BUF_LEN + 1u) * FSMSW_SPHINCSSHA2_192FSIMPLE_N];
  uint32 heights[FSMSW_SPHINCSSHA2_192FSIMPLE_TREEHASH_BUF_LEN + 1u];
  uint32 offset = 0;
  uint32 idx;
  uint32 tree_idx;
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * FSMSW_SPHINCSSHA2_192FSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * FSMSW_SPHINCSSHA2_192FSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsSha2_192fSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHA2_192FSIMPLE_N],
                                         &stack[(offset - 2u) * FSMSW_SPHINCSSHA2_192FSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * FSMSW_SPHINCSSHA2_192FSIMPLE_N],
                               &stack[(offset - 1u) * FSMSW_SPHINCSSHA2_192FSIMPLE_N], FSMSW_SPHINCSSHA2_192FSIMPLE_N);
      }
    }
  }
  FsmSw_CommonLib_MemCpy(root, stack, FSMSW_SPHINCSSHA2_192FSIMPLE_N);
} // end: FsmSw_SphincsSha2_192fSimple_TreeHash
