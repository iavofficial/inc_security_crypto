 ...
 ** @{ */
/** \addtogroup FsmSw_Hqc256_gf
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_Hqc256_gf.c
 ...
 */
static uint16 hqc256_trailing_zero_bits_count(uint16 a)
{
  uint16 bit  = 0x0000;
  uint16 tmp  = 0;
  uint16 mask = 0xFFFF;
 ...
 */
static uint16 hqc256_gf_reduce(uint64 x, uint8 deg_x)
{
  ...
  const uint8 steps = CEIL_DIVIDE(deg_x - (HQC256_PARAM_M - 1), HQC256_PARAM_GF_POLY_M2);

  // Reduce
  for (uint8 i = 0; i < steps; ++i)
  {
    mod = x_tmp >> HQC256_PARAM_M;
    x_tmp &= ((uint64)1 << HQC256_PARAM_M) - 1;
    x_tmp ^= mod;

    z1   = 0;
    rmdr = (uint16)HQC256_PARAM_GF_POLY ^ 0x0001U;
    for (sint8 j = HQC256_PARAM_GF_POLY_WT - 2; j > 0; --j)
    {
      z2   = hqc256_trailing_zero_bits_count(rmdr);
      dist = z2 - z1;
      mod <<= dist;
      x_tmp ^= mod;
      rmdr ^= (uint16)1 << z2;
...
} // end: FsmSw_Hqc256_Gf_Mul

/*====================================================================================================================*/
/**
 * \brief Squares an element of GF(2^HQC256_PARAM_M).
 * \returns a^2
 * \param[in] a Element of GF(2^HQC256_PARAM_M)
 */
uint16 FsmSw_Hqc256_Gf_Square(uint16 a)
{
  uint32 b = a;
  uint32 s = b & (uint32)1;
  for (uint8 i = 1; i < HQC256_PARAM_M; ++i)
  {
    b <<= 1;
    s ^= b & ((uint32)1 << (2 * i));
  }
...
} // end: FsmSw_Hqc256_Gf_Square

/*====================================================================================================================*/
/**
 * \brief Computes the inverse of an element of GF(2^HQC256_PARAM_M),
 * using the addition chain 1 2 3 4 7 11 15 30 60 120 127 254
 * \returns the inverse of a if a != 0 or 0 if a = 0
 * \param[in] a Element of GF(2^HQC256_PARAM_M)
 */
uint16 FsmSw_Hqc256_Gf_Inverse(uint16 a)
{
  uint16 inv = a;
  uint16 tmp1, tmp2;

  inv  = FsmSw_Hqc256_Gf_Square(a);      /* a^2 */
  tmp1 = FsmSw_Hqc256_Gf_Mul(inv, a);    /* a^3 */
  inv  = FsmSw_Hqc256_Gf_Square(inv);    /* a^4 */
  tmp2 = FsmSw_Hqc256_Gf_Mul(inv, tmp1); /* a^7 */
  tmp1 = FsmSw_Hqc256_Gf_Mul(inv, tmp2); /* a^11 */
  inv  = FsmSw_Hqc256_Gf_Mul(tmp1, inv); /* a^15 */
  inv  = FsmSw_Hqc256_Gf_Square(inv);    /* a^30 */
  inv  = FsmSw_Hqc256_Gf_Square(inv);    /* a^60 */
  inv  = FsmSw_Hqc256_Gf_Square(inv);    /* a^120 */
  inv  = FsmSw_Hqc256_Gf_Mul(inv, tmp2); /* a^127 */
  inv  = FsmSw_Hqc256_Gf_Square(inv);    /* a^254 */
  return inv;
} // end: FsmSw_Hqc256_Gf_Inverse
