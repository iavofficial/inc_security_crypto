 ...
 ** @{ */
/** \addtogroup SphincsShake_128sSimple_merkle
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128sSimple_merkle.c
* \brief  description of FsmSw_SphincsShake_128sSimple_merkle.c
*
* \details
*
*
 ...
 *
 */
void FsmSw_SphincsShake_128sSimple_Merkle_Sign(uint8 *const sig, uint8 *const root,
                                               const sphincs_shake_128s_ctx *const ctx, const uint32 wots_addr[8],
                                               uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *const auth_path = &sig[FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_BYTES];

  Fsmsw_Sphincsshake_128sSimple_LeafInfoX1_T info = {
      ((void *)0),
  ...
  uint32 steps[FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_LEN] = {0};

  info.wots_sig = sig;
  FsmSw_SphincsShake_128sSimple_Wots_ChainLengths(steps, root);
  ...
  FsmSw_SphincsShake_128sSimple_TreeHashX1(root, auth_path, ctx, idx_leaf, 0, FSMSW_SPHINCSSHAKE_128SSIMPLE_TREE_HEIGHT,
                                           FsmSw_SphincsShake_128sSimple_Wots_GenLeafX1, tree_addr, &info);
} // end: FsmSw_SphincsShake_128sSimple_Merkle_Sign
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_128sSimple_Merkle_GenRoot(uint8 *const root, const sphincs_shake_128s_ctx *const ctx)
{
  ...
  uint8 auth_path[(FSMSW_SPHINCSSHAKE_128SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHAKE_128SSIMPLE_N) +
                  FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_BYTES] = {0};
  uint32 top_tree_addr[FSMSW_SPHINCS_ADDR_SIZE]             = {0};
  uint32 wots_addr[FSMSW_SPHINCS_ADDR_SIZE]                 = {0};

  FsmSw_SphincsShake_SetLayerAddr(top_tree_addr, FSMSW_SPHINCSSHAKE_128SSIMPLE_D - 1u);
  FsmSw_SphincsShake_SetLayerAddr(wots_addr, FSMSW_SPHINCSSHAKE_128SSIMPLE_D - 1u);

  FsmSw_SphincsShake_128sSimple_Merkle_Sign(auth_path, root, ctx, wots_addr, top_tree_addr,
                                            ~((uint32)0u) /* ~0 means "don't bother generating an auth path */);
} // end: FsmSw_SphincsShake_128sSimple_Merkle_GenRoot
