 ...
 */
static const uint16 gf_log_192[256] = {
...
static void radix_big_192(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f);
...
*
* \param[out] betas Array of size HQC192_PARAM_M-1
*
*/
static void hqc192_compute_fft_betas(uint16 *const betas)
{
  uint8 i;
  for (i = 0; i < (HQC192_PARAM_M - 1); ++i)
  {
    betas[i] = (uint16)1 << (HQC192_PARAM_M - 1 - i);
  }
...
*
*/
static void compute_subset_sums_192(uint16 *const subset_sums, const uint16 *const set, uint16 set_size)
{
  uint16 i, j;
  subset_sums[0] = 0;
...
/*====================================================================================================================*/
/**
* \brief Computes the hqc192_radix conversion of a polynomial f in GF(2^m)[x]
 *
...
static void hqc192_radix(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f)
{
  if (m_f == 4U)
  {
    f0[4] = f[8] ^ f[12];
...
static void radix_big_192(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f)
{
  uint16 Q[(2 * (1U << (HQC192_PARAM_FFT - 2))) + 1] = {0};
  uint16 R[(2 * (1U << (HQC192_PARAM_FFT - 2))) + 1] = {0};

  uint16 Q0[1U << (HQC192_PARAM_FFT - 2)] = {0};
  uint16 Q1[1U << (HQC192_PARAM_FFT - 2)] = {0};
  uint16 R0[1U << (HQC192_PARAM_FFT - 2)] = {0};
  uint16 R1[1U << (HQC192_PARAM_FFT - 2)] = {0};
...
*
*/
static void fft_rec_192(uint16 *const w, uint16 *const f, uint32 f_coeffs, uint8 m, uint32 m_f,
                        const uint16 *const betas)
{
  uint16 f0[1U << (HQC192_PARAM_FFT - 2)]             = {0};
  uint16 f1[1U << (HQC192_PARAM_FFT - 2)]             = {0};
  uint16 gammas[HQC192_PARAM_M - 2]                   = {0};
  uint16 deltas[HQC192_PARAM_M - 2]                   = {0};
  uint16 gammas_sums[1U << (HQC192_PARAM_M - 2)]      = {0};
  uint16 u[1U << (HQC192_PARAM_M - 2)]                = {0};
  uint16 v[1U << (HQC192_PARAM_M - 2)]                = {0};
  uint16 tmp[HQC192_PARAM_M - (HQC192_PARAM_FFT - 1)] = {0};
  ...
  else
  {
    ...
    /* polyspace +2 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fft_rec_192(u, f0, (f_coeffs + 1) / 2, m - 1, m_f - 1, deltas);
    ...
    else
    {
      ...
      // Step 6
      FsmSw_CommonLib_MemCpy(&w[k], v, FsmSw_Convert_u64_to_u32((uint64)2 * k));
      w[0] = u[0];
      w[k] ^= u[0];
      for (i = 1; i < k; ++i)
      {
 ...
 * Note that on this first call (as opposed to the recursive calls to fft_rec_192), gammas are equal to betas,
...
*
* \param[out] w Array
* \param[in]  f Array of 2^HQC192_PARAM_FFT elements
...
void FsmSw_Hqc192_Fft(uint16 *const w, const uint16 *const f, uint32 f_coeffs)
{
  ...
  uint16 f0[1U << (HQC192_PARAM_FFT - 1)]       = {0};
  uint16 f1[1U << (HQC192_PARAM_FFT - 1)]       = {0};
  uint16 deltas[HQC192_PARAM_M - 1]             = {0};
  uint16 u[1U << (HQC192_PARAM_M - 1)]          = {0};
  uint16 v[1U << (HQC192_PARAM_M - 1)]          = {0};

  uint8 i, k;

  // Follows Gao and Mateer algorithm
  hqc192_compute_fft_betas(betas);

  // Step 1: HQC192_PARAM_FFT > 1, nothing to do

  // Compute gammas sums
  compute_subset_sums_192(betas_sums, betas, HQC192_PARAM_M - 1);

  // Step 2: beta_m = 1, nothing to do

  // Step 3
  hqc192_radix(f0, f1, f, HQC192_PARAM_FFT);
  ...
  // Step 5
  fft_rec_192(u, f0, (f_coeffs + 1) / 2, HQC192_PARAM_M - 1, HQC192_PARAM_FFT - 1, deltas);
  fft_rec_192(v, f1, f_coeffs / 2, HQC192_PARAM_M - 1, HQC192_PARAM_FFT - 1, deltas);
...
void FsmSw_Hqc192_Fft_Retrieve_Error_Poly(uint8 *const err, const uint16 *const w)
{
  ...
  hqc192_compute_fft_betas(gammas);
  compute_subset_sums_192(gammas_sums, gammas, HQC192_PARAM_M - 1);

  k = 1U << (HQC192_PARAM_M - 1);
  err[0] ^= (uint8)((1U ^ (((uint16)(~w[0]) + 1U) >> 15)) & 0xFFU);
  err[0] ^= (uint8)((1U ^ (((uint16)(~w[k]) + 1U) >> 15)) & 0xFFU);

  for (i = 1; i < k; ++i)
  {
    index = HQC192_PARAM_GF_MUL_ORDER - gf_log_192[gammas_sums[i]];
