...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256fSimple_hash_sha2.c
* \brief  description of FsmSw_SphincsSha2_256fSimple_hash_sha2.c
*
* \details
*
*
...
#define mgf1_X                FsmSw_SphincsSha2_256fSimple_MgF1512
#define shaXstate             sha512ctx

#define SPX_TREE_BITS  (FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT * (FSMSW_SPHINCSSHA2_256FSIMPLE_D - 1u))
#define SPX_TREE_BYTES ((SPX_TREE_BITS + 7u) / 8u)
#define SPX_LEAF_BITS  FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT
#define SPX_LEAF_BYTES ((SPX_LEAF_BITS + 7u) / 8u)
#define SPX_DGST_BYTES (FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
...
#define SPX_INBLOCKS                                                                                                   \
  (uint32)((uint32)((uint32)(((FSMSW_SPHINCSSHA2_256FSIMPLE_N + FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES +                \
 ...
 */
void FsmSw_SphincsSha2_256fSimple_MgF1256(uint8 *const out, uint32 outlen, const uint8 *const in, uint32 inlen)
{
  uint8 inbuf[FSMSW_SPHINCSSHA2_256FSIMPLE_MGF1_256_BUF_LEN + 4u] = {0};
...
} // end: FsmSw_SphincsSha2_256fSimple_MgF1256
...
hence it is justified." */
void FsmSw_SphincsSha2_256fSimple_MgF1512(uint8 *const out, uint32 outlen, const uint8 *const in, uint32 inlen)
{
  uint8 inbuf[FSMSW_SPHINCSSHA2_256FSIMPLE_MGF1_512_BUF_LEN + 4u] = {0};
...
} // end: FsmSw_SphincsSha2_256fSimple_MgF1512
                                          ...
                                          const uint32 addr[8])
{
  ...
  uint8 buf[SPX_SHA256_ADDR_BYTES + FSMSW_SPHINCSSHA2_256FSIMPLE_N] = {0};
  uint8 outbuf[FSMSW_SPHINCS_SHA256_OUTPUT_BYTES]                   = {0};
  ...
  FsmSw_CommonLib_MemCpy(&buf[SPX_SHA256_ADDR_BYTES], ctx->sk_seed, FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  FsmSw_Sha256_IncFinalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  FsmSw_CommonLib_MemCpy(out, outbuf, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
} // end: FsmSw_SphincsSha2_256fSimple_PrfAddrs
 ...
 *        FSMSW_SPHINCSSHA2_256FSIMPLE_N space available in front of the pointer, i.e. before the message to use
                                                   ...
                                                   const sphincs_sha2_256f_ctx *const ctx)
{
  ...
  FsmSw_CommonLib_MemSet(&buf[FSMSW_SPHINCSSHA2_256FSIMPLE_N], 0x36u,
                         SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  shaX_inc_init(&state);
  shaX_inc_blocks(&state, buf, 1);

  FsmSw_CommonLib_MemCpy(buf, optrand, FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  /* If optrand + message cannot fill up an entire block */
  if ((FSMSW_SPHINCSSHA2_256FSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
  {
    FsmSw_CommonLib_MemCpy(&buf[FSMSW_SPHINCSSHA2_256FSIMPLE_N], m_temp, mlen_temp);
    shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state, buf, mlen_temp + FSMSW_SPHINCSSHA2_256FSIMPLE_N);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    FsmSw_CommonLib_MemCpy(&buf[FSMSW_SPHINCSSHA2_256FSIMPLE_N], m_temp,
                           SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256FSIMPLE_N);
    shaX_inc_blocks(&state, buf, 1);

    m_temp = &m_temp[SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256FSIMPLE_N];
    mlen_temp -= SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256FSIMPLE_N;
  ...
  FsmSw_CommonLib_MemSet(&buf[FSMSW_SPHINCSSHA2_256FSIMPLE_N], 0x5C,
                         SPX_SHAX_BLOCK_BYTES - FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  shaX(buf, buf, SPX_SHAX_BLOCK_BYTES + SPX_SHAX_OUTPUT_BYTES);
  FsmSw_CommonLib_MemCpy(R, buf, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
} // end: FsmSw_SphincsSha2_256fSimple_GenMessageRandom
...
{
  (void)ctx;
  uint8 seed[(2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES] = {0};
  ...
  FsmSw_CommonLib_MemCpy(inbuf, R, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
  FsmSw_CommonLib_MemCpy(&inbuf[FSMSW_SPHINCSSHA2_256FSIMPLE_N], pk, FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES);

  /* If R + pk + message cannot fill up an entire block */
  /* polyspace +3 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
  and therefore not dead code" */
  if ((FSMSW_SPHINCSSHA2_256FSIMPLE_N + FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES + mlen_temp) <
      (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES))
  {
    FsmSw_CommonLib_MemCpy(&inbuf[FSMSW_SPHINCSSHA2_256FSIMPLE_N + FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES], m_temp,
                           mlen_temp);
    shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N], &state, inbuf,
                      FSMSW_SPHINCSSHA2_256FSIMPLE_N + FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES + mlen_temp);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    ...
    and therefore not dead code" */
    FsmSw_CommonLib_MemCpy(&inbuf[FSMSW_SPHINCSSHA2_256FSIMPLE_N + FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES], m_temp,
                           (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_256FSIMPLE_N -
                               FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES);
    shaX_inc_blocks(&state, inbuf, SPX_INBLOCKS);

    /* polyspace +2 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
    and therefore not dead code" */
    m_temp = &m_temp[(SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_256FSIMPLE_N -
                     FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES];
    /* polyspace +3 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change '
    and therefore not dead code" */
    mlen_temp -=
        (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_256FSIMPLE_N - FSMSW_SPHINCSSHA2_256FSIMPLE_PK_BYTES;
    shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N], &state, m_temp, (uint32)mlen_temp);
  }

  /* H_msg: MGF1-SHA-X(R ‖ PK.seed ‖ seed) */
  FsmSw_CommonLib_MemCpy(seed, R, FSMSW_SPHINCSSHA2_256FSIMPLE_N);
  FsmSw_CommonLib_MemCpy(&seed[FSMSW_SPHINCSSHA2_256FSIMPLE_N], pk, FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  /* By doing this in two steps, we prevent hashing the message twice;
     otherwise each iteration in MGF1 would hash the message again. */
  mgf1_X(bufp, SPX_DGST_BYTES, seed, (2u * FSMSW_SPHINCSSHA2_256FSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES);

  FsmSw_CommonLib_MemCpy(digest, bufp, FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_MSG_BYTES);
  bufp = &bufp[FSMSW_SPHINCSSHA2_256FSIMPLE_FORS_MSG_BYTES];
...
} // end: FsmSw_SphincsSha2_256fSimple_HashMessage
