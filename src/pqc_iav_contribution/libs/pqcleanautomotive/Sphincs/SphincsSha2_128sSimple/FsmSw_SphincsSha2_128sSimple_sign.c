...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_128sSimple_sign.c
* \brief  description of FsmSw_SphincsSha2_128sSimple_sign.c
*
* \details
*
*
...
#include "FsmSw_SphincsSha2_128sSimple_FctWrapper.h"
#include "FsmSw_SphincsSha2_128sSimple_context.h"
#include "FsmSw_SphincsSha2_128sSimple_fors.h"
...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_128ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
...
static void fsmsw_sphincssha2_128ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                 const uint8 *const seed)
{
  sphincs_sha2_128s_ctx ctx = {{0}};

  /* Initialize SK_SEED, SK_PRF and PUB_SEED from seed. */
  FsmSw_CommonLib_MemCpy(sk, seed, FSMSW_SPHINCSSHA2_128SSIMPLE_CRYPTO_SEEDBYTES);

  FsmSw_CommonLib_MemCpy(pk, &sk[2u * FSMSW_SPHINCSSHA2_128SSIMPLE_N], FSMSW_SPHINCSSHA2_128SSIMPLE_N);
  ...
  FsmSw_SphincsSha2_128sSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  FsmSw_SphincsSha2_128sSimple_Merkle_GenRoot(&sk[3u * FSMSW_SPHINCSSHA2_128SSIMPLE_N], &ctx);

  FsmSw_CommonLib_MemCpy(&pk[FSMSW_SPHINCSSHA2_128SSIMPLE_N], &sk[3u * FSMSW_SPHINCSSHA2_128SSIMPLE_N],
                         FSMSW_SPHINCSSHA2_128SSIMPLE_N);

  return;
} // end: fsmsw_sphincssha2_128ssimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
...
{
  uint8 seed[FSMSW_SPHINCSSHA2_128SSIMPLE_CRYPTO_SEEDBYTES] = {0};
  (void)FsmSw_CommonLib_RandomBytes(seed, FSMSW_SPHINCSSHA2_128SSIMPLE_CRYPTO_SEEDBYTES);
  fsmsw_sphincssha2_128ssimple_crypto_sign_SeedKeyPair(pk, sk, seed);

  return;
} // end: FsmSw_SphincsSha2_128sSimple_Crypto_Sign_KeyPair
                                                        ...
                                                        uint32 mlen, const uint8 *const sk)
{
  ...
  const uint8 *const sk_prf = &sk[FSMSW_SPHINCSSHA2_128SSIMPLE_N];
  const uint8 *const pk     = &sk[2u * FSMSW_SPHINCSSHA2_128SSIMPLE_N];

  uint8 optrand[FSMSW_SPHINCSSHA2_128SSIMPLE_N]            = {0};
  uint8 mhash[FSMSW_SPHINCSSHA2_128SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[FSMSW_SPHINCSSHA2_128SSIMPLE_N]               = {0};
  uint32 i                                                 = 0;
  uint64 tree                                              = 0;
  uint32 idx_leaf                                          = 0;
  ...
  FsmSw_SphincsSha2_128sSimple_GenMessageRandom(sig_temp, sk_prf, optrand, m, mlen, &ctx);

  /* Derive the message digest and leaf index from R, PK and M. */
  FsmSw_SphincsSha2_128sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_128SSIMPLE_D; i++)
  {
    ...
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_BYTES +
                         (FSMSW_SPHINCSSHA2_128SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_128SSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << FSMSW_SPHINCSSHA2_128SSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> FSMSW_SPHINCSSHA2_128SSIMPLE_TREE_HEIGHT;
  }

  *siglen = FSMSW_SPHINCSSHA2_128SSIMPLE_BYTES;

  return;
} // end: FsmSw_SphincsSha2_128sSimple_Crypto_Sign_Signature
...
{
  sphincs_sha2_128s_ctx ctx                                = {{0}};
  const uint8 *const pub_root                              = &pk[FSMSW_SPHINCSSHA2_128SSIMPLE_N];
  uint8 mhash[FSMSW_SPHINCSSHA2_128SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[FSMSW_SPHINCSSHA2_128SSIMPLE_N]               = {0};
  ...
  FsmSw_SphincsSha2_128sSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128SSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_128SSIMPLE_D; i++)
  {
    ...
    FsmSw_SphincsSha2_128sSimple_copy_keypair_addr(wots_pk_addr, wots_addr);

    /* The WOTS public key is only correct if the signature was correct. */
    /* Initially, root is the FORS pk, but on subsequent iterations it is
       the root of the subtree below the currently processed subtree. */
    FsmSw_SphincsSha2_128sSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_128SSIMPLE_WOTS_BYTES];
    ...
    FsmSw_SphincsSha2_128sSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
  ...
  return retVal;
} // end: FsmSw_SphincsSha2_128sSimple_Crypto_Sign_Verify
                                              ...
                                              const uint8 *const sk)
{
  ...
  (void)FsmSw_SphincsSha2_128sSimple_Crypto_Sign_Signature(sm, &siglen, m, mlen, sk);

  FsmSw_CommonLib_MemMove(&sm[FSMSW_SPHINCSSHA2_128SSIMPLE_BYTES], m, mlen);
  *smlen = siglen + mlen;

  return;
} // end: FsmSw_SphincsSha2_128sSimple_Crypto_Sign
                                                    ...
                                                    uint32 smlen, const uint8 *const pk)
{
  ...
  *mlen = smlen - FSMSW_SPHINCSSHA2_128SSIMPLE_BYTES;
  ...
  return retVal;
} // end: FsmSw_SphincsSha2_128sSimple_Crypto_Sign_Open
