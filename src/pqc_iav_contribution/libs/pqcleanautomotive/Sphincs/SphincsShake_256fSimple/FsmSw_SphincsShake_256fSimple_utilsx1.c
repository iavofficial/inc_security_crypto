...
/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_256fSimple_utilsx1.c
* \brief  description of FsmSw_SphincsShake_256fSimple_utilsx1.c
*
* \details
*
*
...
void FsmSw_SphincsShake_256fSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              ...
                                              void (*const gen_leaf)(uint8 *const leave /* Where to write the leaves */,
                                                                     const sphincs_shake_256f_ctx *const ctx /* ctx */,
                                                                     uint32 idx, void *const info),
                                              uint32 tree_addr[8], void *const info)
{
  /* This is where we keep the intermediate nodes */
  uint8 stack[FSMSW_SPHINCSSHAKE_256FSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHAKE_256FSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[FSMSW_SPHINCSSHAKE_256FSIMPLE_N]. We do this to minimize the number of
     * copies needed during a FsmSw_SphincsShake_256fSimple_Thash */
    uint8 current[2u * FSMSW_SPHINCSSHAKE_256FSIMPLE_N];
    gen_leaf(&current[FSMSW_SPHINCSSHAKE_256FSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHAKE_256FSIMPLE_N], FSMSW_SPHINCSSHAKE_256FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                               &current[FSMSW_SPHINCSSHAKE_256FSIMPLE_N], FSMSW_SPHINCSSHAKE_256FSIMPLE_N);
      }
      ...
      uint8 *const left = &stack[h * FSMSW_SPHINCSSHAKE_256FSIMPLE_N];
      FsmSw_CommonLib_MemCpy(&current[0], left, FSMSW_SPHINCSSHAKE_256FSIMPLE_N);
      FsmSw_SphincsShake_256fSimple_Thash(&current[FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                                          &current[0u * FSMSW_SPHINCSSHAKE_256FSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      FsmSw_CommonLib_MemCpy(&stack[h * FSMSW_SPHINCSSHAKE_256FSIMPLE_N], &current[FSMSW_SPHINCSSHAKE_256FSIMPLE_N],
                             FSMSW_SPHINCSSHAKE_256FSIMPLE_N);
    }
  }
} // end: FsmSw_SphincsShake_256fSimple_TreeHashX1
