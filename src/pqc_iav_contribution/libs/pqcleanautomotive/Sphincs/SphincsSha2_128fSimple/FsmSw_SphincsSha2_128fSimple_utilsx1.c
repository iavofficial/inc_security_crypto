 ...
 ** @{ */
/** \addtogroup SphincsSha2_128fSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_128fSimple_utilsx1.c
* \brief  description of FsmSw_SphincsSha2_128fSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsSha2_128fSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                             const sphincs_sha2_128f_ctx *const ctx, uint32 leaf_idx, uint32 idx_offset,
                                             uint32 tree_height,
                                             void (*const gen_leaf)(uint8 *const out /* Where to write the leaves */,
                                                                    const sphincs_sha2_128f_ctx *const ctx, uint32 idx,
                                                                    void *const info),
                                             uint32 tree_addr[8], void *const info)
{
  uint8 stack[FSMSW_SPHINCSSHA2_128FSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHA2_128FSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[FSMSW_SPHINCSSHA2_128FSIMPLE_N]. We do this to minimize the number of copies
     * needed during a FsmSw_SphincsSha2_128fSimple_Thash */
    uint8 current[2u * FSMSW_SPHINCSSHA2_128FSIMPLE_N];
    gen_leaf(&current[FSMSW_SPHINCSSHA2_128FSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    uint32 h; /* The height we are in the Merkle tree */
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHA2_128FSIMPLE_N], FSMSW_SPHINCSSHA2_128FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHA2_128FSIMPLE_N], &current[FSMSW_SPHINCSSHA2_128FSIMPLE_N],
      ...
      uint8 *const left = &stack[(h * FSMSW_SPHINCSSHA2_128FSIMPLE_N)];
      FsmSw_CommonLib_MemCpy(&current[0], left, FSMSW_SPHINCSSHA2_128FSIMPLE_N);
      FsmSw_SphincsSha2_128fSimple_Thash(&current[FSMSW_SPHINCSSHA2_128FSIMPLE_N],
                                         &current[0u * FSMSW_SPHINCSSHA2_128FSIMPLE_N], 2u, ctx, tree_addr);
      internal_idx >>= 1;
      internal_leaf >>= 1;
    }
    ...
    if (FALSE == bStopFunc)
    {
      ...
      FsmSw_CommonLib_MemCpy(&stack[h * FSMSW_SPHINCSSHA2_128FSIMPLE_N], &current[FSMSW_SPHINCSSHA2_128FSIMPLE_N],
                             FSMSW_SPHINCSSHA2_128FSIMPLE_N);
    }
  }
} // end: FsmSw_SphincsSha2_128fSimple_TreeHashX1
