 ...
 ** @{ */
/** \addtogroup SphincsSha2_128sSimple_hash
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_128sSimple_hash_sha2.c
* \brief  description of FsmSw_SphincsSha2_128sSimple_hash_sha2.c
*
* \details
*
*
                                          ...
                                          const uint32 addr[8])
{
  ...
  FsmSw_Sha256_IncFinalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + FSMSW_SPHINCSSHA2_128SSIMPLE_N);

  FsmSw_CommonLib_MemCpy(out, outbuf, FSMSW_SPHINCSSHA2_128SSIMPLE_N);
} // end: FsmSw_SphincsSha2_128sSimple_PrfAddr
                                                   ...
                                                   const sphincs_sha2_128s_ctx *const ctx)
{
  ...
  if ((FSMSW_SPHINCSSHA2_128SSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
  {
    FsmSw_CommonLib_MemCpy(&buf[FSMSW_SPHINCSSHA2_128SSIMPLE_N], m_temp, mlen_temp);
    shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state, buf, mlen_temp + FSMSW_SPHINCSSHA2_128SSIMPLE_N);
  }
...
} // end: FsmSw_SphincsSha2_128sSimple_GenMessageRandom
                                              ...
                                              uint32 mlen, const sphincs_sha2_128s_ctx *const ctx)
{
  ...
  and therefore not dead code" */
  if ((FSMSW_SPHINCSSHA2_128SSIMPLE_N + FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES + mlen_temp) <
      (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES))
  {
    FsmSw_CommonLib_MemCpy(&inbuf[FSMSW_SPHINCSSHA2_128SSIMPLE_N + FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES], m_temp,
                           mlen_temp);
    shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_128SSIMPLE_N], &state, inbuf,
                      FSMSW_SPHINCSSHA2_128SSIMPLE_N + FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES + mlen_temp);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    ...
    /* polyspace +2 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
    and therefore not dead code" */
    m_temp = &m_temp[(SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_128SSIMPLE_N -
                     FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES];
    /* polyspace +3 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
    and therefore not dead code" */
    mlen_temp -=
        (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - FSMSW_SPHINCSSHA2_128SSIMPLE_N - FSMSW_SPHINCSSHA2_128SSIMPLE_PK_BYTES;
    shaX_inc_finalize(&seed[2u * FSMSW_SPHINCSSHA2_128SSIMPLE_N], &state, m_temp, (uint32)mlen_temp);
  }
...
} // end: FsmSw_SphincsSha2_128sSimple_HashMessage
