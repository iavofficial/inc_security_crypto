...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256sSimple_fors.c
* \brief  description of FsmSw_SphincsSha2_256sSimple_fors.c
*
* \details
*
*
...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_256ssimple_fors_GenSk(uint8 *const sk, const sphincs_sha2_256s_ctx *const ctx,
                                                    const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_256ssimple_fors_SkToLeaf(uint8 *const leaf, const uint8 *const sk,
                                                       const sphincs_sha2_256s_ctx *const ctx,
                                                       const uint32 fors_leaf_addr[8]);
static void fsmsw_sphincssha2_256ssimple_fors_GenLeafx1(uint8 *const leaf, const sphincs_sha2_256s_ctx *const ctx,
                                                        uint32 addr_idx, void *const info);
static void fsmsw_sphincssha2_256ssimple_fors_MessageToIndices(uint32 *const indices, const uint8 *const m);
/**********************************************************************************************************************/
...
static void fsmsw_sphincssha2_256ssimple_fors_GenSk(uint8 *const sk, const sphincs_sha2_256s_ctx *const ctx,
                                                    const uint32 fors_leaf_addr[8])
{
  FsmSw_SphincsSha2_256sSimple_PrfAddr(sk, ctx, fors_leaf_addr);
} // end: fsmsw_sphincssha2_256ssimple_fors_GenSk
...
static void fsmsw_sphincssha2_256ssimple_fors_SkToLeaf(uint8 *const leaf, const uint8 *const sk,
                                                       ...
                                                       const uint32 fors_leaf_addr[8])
{
  FsmSw_SphincsSha2_256sSimple_Thash(leaf, sk, 1, ctx, fors_leaf_addr);
} // end: fsmsw_sphincssha2_256ssimple_fors_SkToLeaf
                                                        ...
                                                        uint32 addr_idx, void *const info)
{
  ...
  /* polyspace +2 MISRA2012:11.5 [Justified:]"Necessary conversion from void* to object* for functionality.
    Ensured proper alignment and validity." */
  Fsmsw_Sphincssha2_256sSimple_ForsGenLeafInfo_T *fors_info = info;
  ...
  fsmsw_sphincssha2_256ssimple_fors_GenSk(leaf, ctx, fors_leaf_addr);

  FsmSw_SphincsSha2_SetType(fors_leaf_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSTREE);
  fsmsw_sphincssha2_256ssimple_fors_SkToLeaf(leaf, leaf, ctx, fors_leaf_addr);
} // end: fsmsw_sphincssha2_256ssimple_fors_GenLeafx1

/*====================================================================================================================*/
/**
 * \brief Interprets m as FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT-bit uint32. Assumes m contains at least
 *        FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT * FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES bits. Assumes indices
 *        has space for FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES integers.
 *
...
static void fsmsw_sphincssha2_256ssimple_fors_MessageToIndices(uint32 *const indices, const uint8 *const m)
{
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES; i++)
  {
    indices[i] = 0;
    for (j = 0; j < FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT; j++)
    {
      indices[i] ^= (uint32)(((((uint32)(m[offset >> 3]) >> (offset & 0x7u)) & 0x1u)) << j);
      offset++;
    }
  }
} // end: fsmsw_sphincssha2_256ssimple_fors_MessageToIndices
/**********************************************************************************************************************/
 ...
 *        FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT * FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES bits.
 *
                                            ...
                                            const sphincs_sha2_256s_ctx *const ctx, const uint32 fors_addr[8])
{
  ...
  FsmSw_SphincsSha2_256sSimple_copy_keypair_addr(fors_tree_addr, fors_addr);
  FsmSw_SphincsSha2_256sSimple_copy_keypair_addr(fors_leaf_addr, fors_addr);

  FsmSw_SphincsSha2_256sSimple_copy_keypair_addr(fors_pk_addr, fors_addr);
  FsmSw_SphincsSha2_SetType(fors_pk_addr, FSMSW_SPHINCS_ADDR_TYPE_FORSPK);

  fsmsw_sphincssha2_256ssimple_fors_MessageToIndices(indices, m);

  for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES; i++)
  {
    idx_offset = i * (uint32)((uint64)((uint64)1u << FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT));
    ...
    fsmsw_sphincssha2_256ssimple_fors_GenSk(sig_temp, ctx, fors_tree_addr);
    ...
    FsmSw_SphincsSha2_256sSimple_TreeHashX1(&roots[i * FSMSW_SPHINCSSHA2_256SSIMPLE_N], sig_temp, ctx, indices[i],
                                            idx_offset, FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT,
                                            fsmsw_sphincssha2_256ssimple_fors_GenLeafx1, fors_tree_addr, &fors_info);

    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256SSIMPLE_N * FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT];
  }

  /* Hash horizontally across all tree roots to derive the public key. */
  FsmSw_SphincsSha2_256sSimple_Thash(pk, roots, FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES, ctx, fors_pk_addr);
} // end: FsmSw_SphincsSha2_256sSimple_Fors_Sign
                                                 ...
                                                 const sphincs_sha2_256s_ctx *const ctx, const uint32 fors_addr[8])
{
  ...
  for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES; i++)
  {
    ...
    fsmsw_sphincssha2_256ssimple_fors_SkToLeaf(leaf, sig_temp, ctx, fors_tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256SSIMPLE_N];

    /* Derive the corresponding root node of this tree. */
    FsmSw_SphincsSha2_256sSimple_ComputeRoot(&roots[i * FSMSW_SPHINCSSHA2_256SSIMPLE_N], leaf, indices[i], idx_offset,
                                             sig_temp, FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT, ctx, fors_tree_addr);
    sig_temp = &sig_temp[FSMSW_SPHINCSSHA2_256SSIMPLE_N * FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_HEIGHT];
  }

  /* Hash horizontally across all tree roots to derive the public key. */
  FsmSw_SphincsSha2_256sSimple_Thash(pk, roots, FSMSW_SPHINCSSHA2_256SSIMPLE_FORS_TREES, ctx, fors_pk_addr);
} // end: FsmSw_SphincsSha2_256sSimple_Fors_PkFromSig
