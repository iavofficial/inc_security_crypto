...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256fSimple_wots.c
* \brief  description of FsmSw_SphincsSha2_256fSimple_wots.c
*
* \details
*
*
...
#include "FsmSw_SphincsSha2_256fSimple_wotsx1.h"
                                                       ...
                                                       uint32 addr[8])
{
  ...
  FsmSw_CommonLib_MemCpy(out, in, FSMSW_SPHINCSSHA2_256FSIMPLE_N);

  /* Iterate 'steps' calls to the hash function. */
  for (i = start; (i < (start + steps)) && (i < FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_W); i++)
  {
    FsmSw_SphincsSha2_SetHashAddr(addr, i);
    FsmSw_SphincsSha2_256fSimple_Thash(out, out, 1, ctx, addr);
  }
} // end: fsmsw_sphincssha2_256fsimple_wots_GenChain
...
static void fsmsw_sphincssha2_256fsimple_wots_BaseW(uint32 *const output, sint32 out_len, const uint8 *const input)
{
  ...
  for (consumed = 0; consumed < out_len; consumed++)
  {
    ...
    bits        = bits - (sint32)FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LOGW;
    output[out] = ((uint32)total >> (uint32)bits) & (FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_W - 1u);
    out++;
  }
} // end: fsmsw_sphincssha2_256fsimple_wots_BaseW
...
static void fsmsw_sphincssha2_256fsimple_wots_Checksum(uint32 *const csum_base_w, const uint32 *const msg_base_w)
{
  ...
  /* Compute checksum. */
  for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN1; i++)
  {
    csum += FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_W - 1u - msg_base_w[i];
  }

  /* Convert checksum to base_w. Make sure expected empty zero bits are the least significant bits. */
  csum = csum << ((8u - ((FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN2 * FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LOGW) % 8u)) % 8u);
  FsmSw_Sphincs_UllToBytes(csum_bytes, sizeof(csum_bytes), csum);
  fsmsw_sphincssha2_256fsimple_wots_BaseW(csum_base_w, (sint32)FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN2, csum_bytes);
} // end: fsmsw_sphincssha2_256fsimple_wots_Checksum
/**********************************************************************************************************************/
...
} // end: FsmSw_SphincsSha2_256fSimple_Wots_ChainLengths
                                                 ...
                                                 const sphincs_sha2_256f_ctx *const ctx, uint32 addr[8])
{
  ...
  FsmSw_SphincsSha2_256fSimple_Wots_ChainLengths(lengths, msg);

  for (i = 0; i < FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN; i++)
  {
    FsmSw_SphincsSha2_SetChainAddr(addr, i);
    fsmsw_sphincssha2_256fsimple_wots_GenChain(&pk[i * FSMSW_SPHINCSSHA2_256FSIMPLE_N],
                                               &sig[i * FSMSW_SPHINCSSHA2_256FSIMPLE_N], lengths[i],
                                               FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_W - 1u - lengths[i], ctx, addr);
  }
} // end: FsmSw_SphincsSha2_256fSimple_Wots_PkFromSig
