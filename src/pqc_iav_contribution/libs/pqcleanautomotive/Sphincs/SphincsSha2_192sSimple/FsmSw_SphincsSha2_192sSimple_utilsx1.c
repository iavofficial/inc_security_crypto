...
/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_192sSimple_utilsx1.c
* \brief  description of FsmSw_SphincsSha2_192sSimple_utilsx1.c
*
* \details
*
*
...
void FsmSw_SphincsSha2_192sSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                             ...
                                             void (*const gen_leaf)(uint8 *const out /* Where to write the leaves */,
                                                                    const sphincs_sha2_192s_ctx *const ctx, uint32 idx,
                                                                    void *const info),
                                             uint32 tree_addr[8], void *const info)
{
  /* This is where we keep the intermediate nodes */
  uint8 stack[FSMSW_SPHINCSSHA2_192SSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHA2_192SSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[FSMSW_SPHINCSSHA2_192SSIMPLE_N].  We do this to minimize the number of
     * copies needed during a FsmSw_SphincsSha2_192sSimple_Thash */
    uint8 current[2u * FSMSW_SPHINCSSHA2_192SSIMPLE_N];
    gen_leaf(&current[FSMSW_SPHINCSSHA2_192SSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHA2_192SSIMPLE_N], FSMSW_SPHINCSSHA2_192SSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHA2_192SSIMPLE_N], &current[FSMSW_SPHINCSSHA2_192SSIMPLE_N],
      ...
      uint8 const *const left = &stack[h * FSMSW_SPHINCSSHA2_192SSIMPLE_N];
      FsmSw_CommonLib_MemCpy(&current[0], left, FSMSW_SPHINCSSHA2_192SSIMPLE_N);
      FsmSw_SphincsSha2_192sSimple_Thash(&current[FSMSW_SPHINCSSHA2_192SSIMPLE_N],
                                         &current[0u * FSMSW_SPHINCSSHA2_192SSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      FsmSw_CommonLib_MemCpy(&stack[h * FSMSW_SPHINCSSHA2_192SSIMPLE_N], &current[FSMSW_SPHINCSSHA2_192SSIMPLE_N],
                             FSMSW_SPHINCSSHA2_192SSIMPLE_N);
    }
  }
} // end: FsmSw_SphincsSha2_192sSimple_TreeHashX1
