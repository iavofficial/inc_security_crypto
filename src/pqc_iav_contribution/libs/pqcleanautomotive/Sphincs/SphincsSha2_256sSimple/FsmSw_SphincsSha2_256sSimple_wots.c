...
/**********************************************************************************************************************/
static void fsmsw_sphincssha2_256ssimple_wots_GenChain(uint8 *const out, const uint8 *const in, uint32 start,
                                                       uint32 steps, const sphincs_sha2_256s_ctx *const ctx,
                                                       uint32 addr[8]);
static void fsmsw_sphincssha2_256ssimple_wots_BaseW(uint32 *const output, sint32 out_len, const uint8 *const input);
static void fsmsw_sphincssha2_256ssimple_wots_Checksum(uint32 *const csum_base_w, const uint32 *const msg_base_w);
/**********************************************************************************************************************/
                                                       ...
                                                       uint32 addr[8])
{
  ...
  FsmSw_CommonLib_MemCpy(out, in, FSMSW_SPHINCSSHA2_256SSIMPLE_N);

  /* Iterate 'steps' calls to the hash function. */
  for (i = start; (i < (start + steps)) && (i < FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_W); i++)
  {
...
static void fsmsw_sphincssha2_256ssimple_wots_BaseW(uint32 *const output, sint32 out_len, const uint8 *const input)
{
  ...
  for (consumed = 0; consumed < out_len; consumed++)
  {
    ...
    bits        = bits - (sint32)FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LOGW;
...
{
  uint32 csum                                                                                                     = 0;
  uint8 csum_bytes[((FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN2 * FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LOGW) + 7u) / 8u] = {0};
  uint32 i                                                                                                        = 0;

  /* Compute checksum. */
  for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN1; i++)
  {
    csum += FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_W - 1u - msg_base_w[i];
  }
...
{
  uint32 lengths[FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN] = {0};
  uint32 i                                              = 0;

  FsmSw_SphincsSha2_256sSimple_Wots_ChainLengths(lengths, msg);

  for (i = 0; i < FSMSW_SPHINCSSHA2_256SSIMPLE_WOTS_LEN; i++)
  {
    FsmSw_SphincsSha2_SetChainAddr(addr, i);
    fsmsw_sphincssha2_256ssimple_wots_GenChain(&pk[i * FSMSW_SPHINCSSHA2_256SSIMPLE_N],
