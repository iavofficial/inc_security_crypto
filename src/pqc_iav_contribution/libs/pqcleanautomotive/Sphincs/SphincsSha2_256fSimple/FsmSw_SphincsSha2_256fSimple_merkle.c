 ...
 ** @{ */
/** \addtogroup SphincsSha2_256fSimple_merkle
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_256fSimple_merkle.c
* \brief  description of FsmSw_SphincsSha2_256fSimple_merkle.c
*
* \details
*
*
 ...
 * \param[in]  uint32                  idx_leaf  .b.d.
 *
 */
void FsmSw_SphincsSha2_256fSimple_Merkle_Sign(uint8 *const sig, uint8 *const root,
                                              const sphincs_sha2_256f_ctx *const ctx, const uint32 wots_addr[8],
                                              uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *const auth_path = &sig[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES];

  Fsmsw_Sphincssha2_256fSimple_LeafInfoX1_T info = {
      ((void *)0),
  ...
  uint32 steps[FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_LEN] = {0};

  info.wots_sig = sig;
  FsmSw_SphincsSha2_256fSimple_Wots_ChainLengths(steps, root);
  ...
  FsmSw_SphincsSha2_256fSimple_TreeHashX1(root, auth_path, ctx, idx_leaf, 0, FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT,
                                          FsmSw_SphincsSha2_256fSimple_Wots_GenLeafX1, tree_addr, &info);
} // end: FsmSw_SphincsSha2_256fSimple_Merkle_Sign
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsSha2_256fSimple_Merkle_GenRoot(uint8 *const root, const sphincs_sha2_256f_ctx *const ctx)
{
  ...
  uint8 auth_path[(FSMSW_SPHINCSSHA2_256FSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_256FSIMPLE_N) +
                  FSMSW_SPHINCSSHA2_256FSIMPLE_WOTS_BYTES] = {0};
  uint32 top_tree_addr[FSMSW_SPHINCS_ADDR_SIZE]            = {0};
  uint32 wots_addr[FSMSW_SPHINCS_ADDR_SIZE]                = {0};

  FsmSw_SphincsSha2_SetLayerAddr(top_tree_addr, FSMSW_SPHINCSSHA2_256FSIMPLE_D - 1u);
  FsmSw_SphincsSha2_SetLayerAddr(wots_addr, FSMSW_SPHINCSSHA2_256FSIMPLE_D - 1u);

  FsmSw_SphincsSha2_256fSimple_Merkle_Sign(auth_path, root, ctx, wots_addr, top_tree_addr,
                                           ~((uint32)0u) /* ~0 means "don't bother generating an auth path */);
} // end: FsmSw_SphincsSha2_256fSimple_Merkle_GenRoot
