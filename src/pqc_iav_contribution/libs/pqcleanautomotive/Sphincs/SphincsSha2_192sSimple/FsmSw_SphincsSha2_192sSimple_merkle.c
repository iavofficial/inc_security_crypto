 ...
 ** @{ */
/** \addtogroup SphincsSha2_192sSimple_merkle
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsSha2_192sSimple_merkle.c
* \brief  description of FsmSw_SphincsSha2_192sSimple_merkle.c
*
* \details
*
*
 ...
 *
 */
void FsmSw_SphincsSha2_192sSimple_Merkle_Sign(uint8 *const sig, uint8 *const root,
                                              const sphincs_sha2_192s_ctx *const ctx, const uint32 wots_addr[8],
                                              uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *const auth_path = &sig[FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_BYTES];

  Fsmsw_Sphincssha2_192sSimple_LeafInfoX1_T info = {
      ((void *)0),
  ...
  uint32 steps[FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_LEN] = {0};

  info.wots_sig = sig;
  FsmSw_SphincsSha2_192sSimple_Wots_ChainLengths(steps, root);
  ...
  FsmSw_SphincsSha2_192sSimple_TreeHashX1(root, auth_path, ctx, idx_leaf, 0, FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT,
                                          FsmSw_SphincsSha2_192sSimple_Wots_GenLeafX1, tree_addr, &info);
} // end: FsmSw_SphincsSha2_192sSimple_Merkle_Sign
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsSha2_192sSimple_Merkle_GenRoot(uint8 *const root, const sphincs_sha2_192s_ctx *const ctx)
{
  /* We do not need the auth path in key generation, but it simplifies the
     code to have just one treehash routine that computes both root and path
     in one function. */
  uint8 auth_path[(FSMSW_SPHINCSSHA2_192SSIMPLE_TREE_HEIGHT * FSMSW_SPHINCSSHA2_192SSIMPLE_N) +
                  FSMSW_SPHINCSSHA2_192SSIMPLE_WOTS_BYTES] = {0};
  uint32 top_tree_addr[FSMSW_SPHINCS_ADDR_SIZE]            = {0};
  uint32 wots_addr[FSMSW_SPHINCS_ADDR_SIZE]                = {0};

  FsmSw_SphincsSha2_SetLayerAddr(top_tree_addr, FSMSW_SPHINCSSHA2_192SSIMPLE_D - 1u);
  FsmSw_SphincsSha2_SetLayerAddr(wots_addr, FSMSW_SPHINCSSHA2_192SSIMPLE_D - 1u);

  FsmSw_SphincsSha2_192sSimple_Merkle_Sign(auth_path, root, ctx, wots_addr, top_tree_addr,
                                           ~((uint32)0u) /* ~0 means "don't bother generating an auth path */);
} // end: FsmSw_SphincsSha2_192sSimple_Merkle_GenRoot
