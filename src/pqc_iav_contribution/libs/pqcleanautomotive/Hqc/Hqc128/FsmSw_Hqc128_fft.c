...
static void radix_big_128(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f);
 ...
 *
 * \param[out] betas Array of size HQC128_PARAM_M-1
 */
static void hqc128_compute_fft_betas(uint16 *const betas)
{
  uint8 i;
  for (i = 0; i < (HQC128_PARAM_M - 1); ++i)
  {
    betas[i] = (uint16)1 << (HQC128_PARAM_M - 1 - i);
  }
 ...
 */
static void compute_subset_sums_128(uint16 *const subset_sums, const uint16 *const set, uint16 set_size)
{
  uint16 i, j;
  subset_sums[0] = 0;
...
/*====================================================================================================================*/
/**
 * \brief Computes the hqc128_radix conversion of a polynomial f in GF(2^m)[x]
 *
...
static void hqc128_radix(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f)
{
  if (m_f == 4U)
  {
    f0[4] = f[8] ^ f[12];
...
static void radix_big_128(uint16 *const f0, uint16 *const f1, const uint16 *const f, uint32 m_f)
{
  uint16 Q[(2 * (1U << (HQC128_PARAM_FFT - 2))) + 1] = {0};
  uint16 R[(2 * (1U << (HQC128_PARAM_FFT - 2))) + 1] = {0};

  uint16 Q0[1U << (HQC128_PARAM_FFT - 2)] = {0};
  uint16 Q1[1U << (HQC128_PARAM_FFT - 2)] = {0};
  uint16 R0[1U << (HQC128_PARAM_FFT - 2)] = {0};
  uint16 R1[1U << (HQC128_PARAM_FFT - 2)] = {0};
                        ...
                        const uint16 *const betas)
{
  ...
  uint16 f1[1U << (HQC128_PARAM_FFT - 2)]             = {0};
  uint16 gammas[HQC128_PARAM_M - 2]                   = {0};
  uint16 deltas[HQC128_PARAM_M - 2]                   = {0};
  uint16 gammas_sums[1U << (HQC128_PARAM_M - 2)]      = {0};
  uint16 u[1U << (HQC128_PARAM_M - 2)]                = {0};
  uint16 v[1U << (HQC128_PARAM_M - 2)]                = {0};
  uint16 tmp[HQC128_PARAM_M - (HQC128_PARAM_FFT - 1)] = {0};
  ...
  else
  {
    ...
    /* polyspace +2 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fft_rec_128(u, f0, (f_coeffs + 1) / 2, m - 1, m_f - 1, deltas);
 ...
 * Note that on this first call (as opposed to the recursive calls to fft_rec_128), gammas are equal to betas,
...
void FsmSw_Hqc128_Fft(uint16 *const w, const uint16 *const f, uint8 f_coeffs)
{
  ...
  uint16 f0[1U << (HQC128_PARAM_FFT - 1)]       = {0};
  uint16 f1[1U << (HQC128_PARAM_FFT - 1)]       = {0};
  uint16 deltas[HQC128_PARAM_M - 1]             = {0};
  uint16 u[1U << (HQC128_PARAM_M - 1)]          = {0};
  uint16 v[1U << (HQC128_PARAM_M - 1)]          = {0};

  uint8 i, k;

  // Follows Gao and Mateer algorithm
  hqc128_compute_fft_betas(betas);

  // Step 1: HQC128_PARAM_FFT > 1, nothing to do

  // Compute gammas sums
  compute_subset_sums_128(betas_sums, betas, HQC128_PARAM_M - 1);

  // Step 2: beta_m = 1, nothing to do

  // Step 3
  hqc128_radix(f0, f1, f, HQC128_PARAM_FFT);
  ...
  // Step 5
  fft_rec_128(u, f0, (f_coeffs + 1) / 2, HQC128_PARAM_M - 1, HQC128_PARAM_FFT - 1, deltas);
  fft_rec_128(v, f1, f_coeffs / 2, HQC128_PARAM_M - 1, HQC128_PARAM_FFT - 1, deltas);
