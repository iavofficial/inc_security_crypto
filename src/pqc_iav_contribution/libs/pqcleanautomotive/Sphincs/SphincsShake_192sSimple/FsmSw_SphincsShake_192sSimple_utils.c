 ...
 ** @{ */
/** \addtogroup SphincsShake_192sSimple_utils
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_192sSimple_utils.c
* \brief  description of FsmSw_SphincsShake_192sSimple_utils.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_192sSimple_ComputeRoot(uint8 *const root, const uint8 *const leaf, uint32 leaf_idx,
                                               uint32 idx_offset, const uint8 *const auth_path, uint32 tree_height,
                                               const sphincs_shake_192s_ctx *const ctx, uint32 addr[8])
{
  uint32 i                                           = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHAKE_192SSIMPLE_N] = {0};
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_192SSIMPLE_N], leaf, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_192SSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
  }
  auth_path_temp = &auth_path_temp[FSMSW_SPHINCSSHAKE_192SSIMPLE_N];
                                              ...
                                              uint32 tree_addr[8])
{
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_192SSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsShake_192sSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_192SSIMPLE_N],
                                          &stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_192SSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * FSMSW_SPHINCSSHAKE_192SSIMPLE_N],
                               &stack[(offset - 1u) * FSMSW_SPHINCSSHAKE_192SSIMPLE_N],
                               FSMSW_SPHINCSSHAKE_192SSIMPLE_N);
      }
    }
  }
