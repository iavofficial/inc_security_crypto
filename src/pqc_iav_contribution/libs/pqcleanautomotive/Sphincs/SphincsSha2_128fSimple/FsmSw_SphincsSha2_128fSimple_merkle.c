 ...
 *
 */
void FsmSw_SphincsSha2_128fSimple_Merkle_Sign(uint8 *const sig, uint8 *const root,
                                              const sphincs_sha2_128f_ctx *const ctx, const uint32 wots_addr[8],
                                              uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *const auth_path = &sig[FSMSW_SPHINCSSHA2_128FSIMPLE_WOTS_BYTES];
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsSha2_128fSimple_Merkle_GenRoot(uint8 *const root, const sphincs_sha2_128f_ctx *const ctx)
{
