 ...
 ** @{ */
/** \addtogroup SphincsShake_192fSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_192fSimple_utilsx1.c
* \brief  description of FsmSw_SphincsShake_192fSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_192fSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              const sphincs_shake_192f_ctx *const ctx, uint32 leaf_idx,
...
{
  uint8 stack[FSMSW_SPHINCSSHAKE_192FSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHAKE_192FSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHAKE_192FSIMPLE_N], FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      if (((internal_idx ^ internal_leaf) == 0x01u) && (FALSE == bStopFunc))
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHAKE_192FSIMPLE_N],
                               &current[FSMSW_SPHINCSSHAKE_192FSIMPLE_N], FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
      }
      ...
      uint8 *const left = &stack[h * FSMSW_SPHINCSSHAKE_192FSIMPLE_N];
      ...
      FsmSw_SphincsShake_192fSimple_Thash(&current[FSMSW_SPHINCSSHAKE_192FSIMPLE_N],
                                          &current[0u * FSMSW_SPHINCSSHAKE_192FSIMPLE_N], 2u, ctx, tree_addr);
...
} // end: FsmSw_SphincsShake_192fSimple_TreeHashX1
