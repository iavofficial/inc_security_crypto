 ...
 ** @{ */
/** \addtogroup SphincsShake_128sSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_128Simple_utilsx1.c
* \brief  description of FsmSw_SphincsShake_128sSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_128sSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              const sphincs_shake_128s_ctx *const ctx, uint32 leaf_idx,
                                              ...
                                              uint32 tree_addr[8], void *const info)
{
  ...
  uint8 stack[FSMSW_SPHINCSSHAKE_128SSIMPLE_TREEHASHX1_BUF_LEN * FSMSW_SPHINCSSHAKE_128SSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < FSMSW_SPHINCS_UINT32_MAX_VALUE; idx++)
  {
    ...
    gen_leaf(&current[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < FSMSW_SPHINCS_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        FsmSw_CommonLib_MemCpy(root, &current[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      if (((internal_idx ^ internal_leaf) == 0x01u) && (FALSE == bStopFunc))
      {
        FsmSw_CommonLib_MemCpy(&auth_path[h * FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                               &current[FSMSW_SPHINCSSHAKE_128SSIMPLE_N], FSMSW_SPHINCSSHAKE_128SSIMPLE_N);
      }
      ...
      uint8 *const left = &stack[h * FSMSW_SPHINCSSHAKE_128SSIMPLE_N];
      ...
      FsmSw_SphincsShake_128sSimple_Thash(&current[FSMSW_SPHINCSSHAKE_128SSIMPLE_N],
                                          &current[0u * FSMSW_SPHINCSSHAKE_128SSIMPLE_N], 2u, ctx, tree_addr);
...
} // end: FsmSw_SphincsShake_128sSimple_TreeHashX1
