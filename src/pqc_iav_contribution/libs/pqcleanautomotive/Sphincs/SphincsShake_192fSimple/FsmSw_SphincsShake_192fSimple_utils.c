...
/*====================================================================================================================*/
/** \file FsmSw_SphincsShake_192fSimple_utils.c
* \brief  description of FsmSw_SphincsShake_192fSimple_utils.c
*
* \details
*
*
...
{
  uint32 i                                           = 0;
  uint8 buffer[2u * FSMSW_SPHINCSSHAKE_192FSIMPLE_N] = {0};
  ...
  else
  {
    ...
    FsmSw_CommonLib_MemCpy(&buffer[FSMSW_SPHINCSSHAKE_192FSIMPLE_N], auth_path_temp, FSMSW_SPHINCSSHAKE_192FSIMPLE_N);
  }
...
} // end: FsmSw_SphincsShake_192fSimple_ComputeRoot
...
void FsmSw_SphincsShake_192fSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                            ...
                                            uint32 tree_addr[8])
{
  uint8 stack[(FSMSW_SPHINCSSHAKE_192FSIMPLE_TREEHASH_BUF_LEN + 1u) * FSMSW_SPHINCSSHAKE_192FSIMPLE_N] = {0};
  uint32 heights[FSMSW_SPHINCSSHAKE_192FSIMPLE_TREEHASH_BUF_LEN + 1u]                                  = {0};
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      FsmSw_SphincsShake_192fSimple_Thash(&stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_192FSIMPLE_N],
                                          &stack[(offset - 2u) * FSMSW_SPHINCSSHAKE_192FSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
...
} // end: FsmSw_SphincsShake_192fSimple_TreeHash
